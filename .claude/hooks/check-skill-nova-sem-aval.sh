...
# Bypass legítimo: export JADE_CONTEXT='skill-aprovada-pelo-operador=true' antes do Write.
#
  ...
  Write)
    case "$FILE_PATH" in
      */.claude/commands/*.md)
        ...
        if [ "$JADE_CONTEXT" = "skill-aprovada-pelo-operador=true" ]; then
          exit 0
        fi
        cat <<MSG >&2
...
Bypass legítimo (após aval do {{NOME_OPERADOR_CURTO}}): export JADE_CONTEXT='skill-aprovada-pelo-operador=true'
