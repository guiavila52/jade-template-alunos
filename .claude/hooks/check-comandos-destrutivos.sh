...
if echo "$CMD" | grep -qE '\brm\s+(-[a-zA-Z]*r[a-zA-Z]*f|-[a-zA-Z]*f[a-zA-Z]*r)\s+(/|/usr|/etc|/var|/home|\$HOME|~/?\s|\.\s|\.$|\./?\s|\./?\$)' ; then
  BLOCK=1; REASON="rm -rf em path crítico (raiz, /usr, /home, ~, .)"
fi

# Pattern 2: rm -rf .git/ (mata histórico git)
if echo "$CMD" | grep -qE '\brm\s+-[a-zA-Z]*r[a-zA-Z]*f?\s+\.git/?\b' ; then
  BLOCK=1; REASON="rm -rf .git/ (destrói histórico git)"
fi

# Pattern 3: git push --force / -f em main/master
if echo "$CMD" | grep -qE 'git\s+push\s+(.*\s)?(-f|--force|--force-with-lease)\b.*\b(main|master|production|prod)\b' ; then
...
if echo "$CMD" | grep -qiE '\b(drop\s+(database|schema|table)|truncate\s+table)\b' ; then
