...
# Detecta se é PATCH via Gimmick API
if [[ "$BASH_CMD" =~ curl.*-X\ PATCH.*gimmick\.guiavila\.com/api/content/newsletters ]]; then
  
  OUTPUT_DIR="/Users/{{SEU_USUARIO}}/Documents/Projetos IA {{NOME_OPERADOR}}/Squad Empresa {{NOME_OPERADOR}}/workspace/output/newsletter"
  ...
  # Critério 3 — Screenshot Gmail real do {{NOME_OPERADOR}} (< 6h) — OPCIONAL mas avisa
  ...
  if [[ -z "$TESTE_INBOX" ]]; then
    ...
    echo "Hook permite passar, mas recomenda aprovação visual do {{NOME_OPERADOR}} antes de produção."
  fi
