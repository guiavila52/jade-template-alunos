...
# Detecta se é PATCH via API da plataforma de conteúdo
if [[ "$BASH_CMD" =~ curl.*-X\ PATCH.*{{PLATAFORMA_API_URL}}/api/content/newsletters ]]; then
  
  OUTPUT_DIR="$CLAUDE_PROJECT_DIR/workspace/output/newsletter"
  ...
  # Critério 3 — Screenshot Gmail real do {{NOME_OPERADOR_CURTO}} (< 6h) — OPCIONAL mas avisa
  ...
  if [[ -z "$TESTE_INBOX" ]]; then
    ...
    echo "Hook permite passar, mas recomenda aprovação visual do {{NOME_OPERADOR_CURTO}} antes de produção."
  fi
