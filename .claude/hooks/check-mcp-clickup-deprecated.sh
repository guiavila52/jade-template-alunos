...
Use as skills REST (curl direto em api.clickup.com):
  - /criar-pendencia       — cria task na lista Tasks Jade COO ({{CLICKUP_LIST_ID}})
  ...
  - /{{plataforma}}-tasks         — consulta lista Tasks do Operador
  - /clickup-task-done     — encerra task na lista do operador ({{CLICKUP_LIST_ID}})
