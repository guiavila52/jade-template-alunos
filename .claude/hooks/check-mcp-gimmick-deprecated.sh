#!/usr/bin/env bash
# check-mcp-{{plataforma_conteudo}}-deprecated.sh
# Hook PreToolUse — bloqueia qualquer chamada a tools mcp__{{CONTENT_PLATFORM}}__* (MCP descontinuado 12/05/2026).
# Substituto canônico: API REST do {{Plataforma_Conteudo}}.
# Ver: segundo-cerebro/03-operacao/{{plataforma_conteudo}}-historico.md
...
case "${tool_name:-}" in
  mcp__{{CONTENT_PLATFORM}}__*)
    cat >&2 <<MSG
[BLOCK] MCP do {{Plataforma_Conteudo}} descontinuado em 12/05/2026 — tools mcp__{{CONTENT_PLATFORM}}__* retornam HTTP 401 e foram desativadas.

Tool bloqueada: ${tool_name}

Substituto canônico: API REST do {{Plataforma_Conteudo}}.
  - Auth: Authorization: Bearer \${CONTENT_API_KEY}  (sk-sq-*)
  - Base: https://{{DOMINIO_CONTEUDO}}/api/content
  - Endpoints + exemplos curl: segundo-cerebro/03-operacao/{{plataforma_conteudo}}-historico.md
