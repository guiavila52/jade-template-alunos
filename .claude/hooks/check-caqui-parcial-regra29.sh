...
# Bloqueia declarações de "Caqui parcial" ou "aguarda {{NOME_OPERADOR_CURTO}}" sem o checklist obrigatório das 4 categorias.
...
if echo "$COMMAND" | grep -qiE "Caqui parcial|aguarda {{NOME_OPERADOR_CURTO}}|espera {{NOME_OPERADOR_CURTO}} voltar|aguardando {{NOME_OPERADOR_CURTO}}|aguardando aprovação"; then
    ...
    echo "BLOQUEIO Regra Inviolável #29: você está prestes a declarar 'Caqui parcial' / 'aguarda {{NOME_OPERADOR_CURTO}}' SEM ter respondido o checklist das 4 categorias de gate." >&2
    echo "" >&2
    ...
    echo "  [ ] É INPUT EXTERNO físico que só {{NOME_OPERADOR_CURTO}} tem? (cat 3)" >&2
