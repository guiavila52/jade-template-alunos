...
write_patterns = [
    (r'>\s*[\'"]?[^|&;<>\s]*' + esc, 'redirect >'),
    (r'>>\s*[\'"]?[^|&;<>\s]*' + esc, 'redirect >>'),
    (r'tee\s+(-a\s+)?[\'"]?[^|&;<>\s]*' + esc, 'tee'),
    (r"sed\s+-i(\s+''|\s+'[^']*')?\s+[^|&;]*?" + esc, 'sed -i'),
    (r'cat\s+[^|&;]*>\s*[\'"]?[^|&;<>\s]*' + esc, 'cat >'),
    (r'echo\s+[^|&;]*>\s*[\'"]?[^|&;<>\s]*' + esc, 'echo >'),
    (r'printf\s+[^|&;]*>\s*[\'"]?[^|&;<>\s]*' + esc, 'printf >'),
    (r'python3?\s+-c\s+[\'"][^\'"]*(write|w\+|open)[^\'"]*' + esc, 'python -c write'),
    (r'mv\s+[^|&;]*' + esc, 'mv -> protected'),
    (r'cp\s+[^|&;]*' + esc, 'cp -> protected'),
    (r'rm\s+[^|&;]*' + esc, 'rm protected'),
    (r'truncate\s+[^|&;]*' + esc, 'truncate'),
    (r'dd\s+[^|&;]*of=[\'"]?[^|&;<>\s]*' + esc, 'dd of='),
]
