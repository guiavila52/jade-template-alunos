...
# Pendência mãe: ClickUp {{CLICKUP_TASK_EXEMPLO}}
...
if [[ "$RESULT" == BLOCK* ]]; then
  ...
  BYPASS_TOKENS=("skill-aprovada=true" "fase-0-hardening=true" "regra-aprovada-operador=true" "rotina-autonoma")
...
BYPASS LEGITIMO:
- export JADE_CONTEXT=skill-aprovada=true   (Regra §13 — skill nova com aval {{NOME_OPERADOR_CURTO}})
- export JADE_CONTEXT=fase-0-hardening=true (durante execução da Fase 0)
- export JADE_CONTEXT=regra-aprovada-operador=true (mudança de regra com aval explícito)
...
Pendencia mae: ClickUp {{CLICKUP_TASK_EXEMPLO}}
MSG
  exit 2
fi
