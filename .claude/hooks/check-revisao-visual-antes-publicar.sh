...
# 1. PATCH/POST body {{Plataforma_Conteudo}}
if echo "$COMMAND" | grep -qE '{{DOMINIO_CONTEUDO}}/api/content/(newsletters|car[a-z]+|ideias)'; then
  if echo "$COMMAND" | grep -qE '"body"' && echo "$COMMAND" | grep -qE '(PATCH|POST)'; then
    NEEDS_REVIEW=1
    CTX="PATCH/POST body {{Plataforma_Conteudo}}"
  fi
fi
...
if [[ "$NEEDS_REVIEW" -eq 1 ]]; then
  PROJ="~/Documents/Projetos IA Gui Ávila/Squad Empresa Gui Ávila"
