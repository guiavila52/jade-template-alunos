...
# 1. PATCH/POST body {{PLATAFORMA_EMAIL}}
if echo "$COMMAND" | grep -qE '{{PLATAFORMA_API_URL}}/api/content/(newsletters|car[a-z]+|ideias)'; then
  if echo "$COMMAND" | grep -qE '"body"' && echo "$COMMAND" | grep -qE '(PATCH|POST)'; then
    NEEDS_REVIEW=1
    CTX="PATCH/POST body {{PLATAFORMA_EMAIL}}"
  fi
fi
