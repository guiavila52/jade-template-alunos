#!/usr/bin/env bash
# PreToolUse hook — Regra §1 (reforço 16/05/2026, Task {{CLICKUP_TASK_EXEMPLO}})
...
# - JADE_CONTEXT=skill-aprovada=* (criação de skill com aval do {{NOME_OPERADOR_CURTO}}, Regra §13)
...
Pendências SÓ via ClickUp list {{CLICKUP_LIST_ID}} usando skills canônicas:
- /criar-pendencia
- /listar-pendencias
- /comentar-pendencia
- /fechar-pendencia
...
# Heurística: presença de "Task ClickUp: <id>" ou "**Task ClickUp:** <id>" no INPUT
if printf '%s' "$INPUT" | grep -qiE 'task clickup.*[a-z0-9]{6,}'; then
  exit 0
fi
...
ANTES de prosseguir:
1. Registre pendência no ClickUp list {{CLICKUP_LIST_ID}} via /criar-pendencia
...
- JADE_CONTEXT=skill-aprovada={id} (skill nova com aval {{NOME_OPERADOR_CURTO}}, Regra §13)
- Briefing de subagent contendo "Task ClickUp: <id>..." (detectado via stdin)
