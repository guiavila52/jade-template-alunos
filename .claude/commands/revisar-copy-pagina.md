...
3. **Fonte display (Syne, Fraunces, Cormorant) JAMAIS em números/preços/datas/cupons** — sempre Inter Tight com `font-variant-numeric: tabular-nums` OU JetBrains Mono. Display em números gera kerning estranho e baseline desalinhada. Reincidência banida pelo Gui em 18/05/2026 — ver memória `feedback_fonte_display_jamais_em_numeros.md`
...
- [ ] **Métricas públicas — sem faturamento**:
  - [ ] grep `-E "(R\\$|fatura|faturamento|MRR|receita)"` no `.astro` retorna 0 menções relacionadas a empresas do Gui ({{EMPRESA_COFUNDADA}}, {{EMPRESA_NEGOCIO}}, {{MARCA_PESSOAL}}, {{EMPRESA_HOLDING}})
...
- [ ] **Prova social — honesta, sobre o GUI, inequívoca**:
  - [ ] grep `400 ?k|400 ?mil` retorna 0 ocorrências como métrica do Gui (pode aparecer descrevendo a {{EMPRESA_COFUNDADA}} em contexto claro: "plataforma com 400k+ usuários hospedando cursos")
