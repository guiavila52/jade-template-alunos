---
name: revisar-pagina
description: Audita copy de pagina antes do codigo (Light Copy, Cormorant em digitos, vagueza calibrada, prova social) — squad-copy.
type: skill
---
...
3. **Fonte display (Syne, Fraunces, Cormorant) JAMAIS em números/preços/datas/cupons** — sempre Inter Tight com `font-variant-numeric: tabular-nums` OU JetBrains Mono. Display em números gera kerning estranho e baseline desalinhada. Reincidência banida pelo {{NOME_OPERADOR_CURTO}} em 18/05/2026 — ver memória `feedback_fonte_display_jamais_em_numeros.md`
...
[4a] REPROVADA                                       [4b] APROVADA
  ...
  seção + problema + sugestão                        com observações positivas
        │                                                   │
        ▼                                                   ▼
[5a] Devolver ao Agente Copy               [5b] Instruir Agente Dev: executar /ajustar-pagina
...
```
/revisar-copy-pagina workspace/output/paginas/YYYY-MM-DD-[slug].md
```
...
- [ ] **Métricas públicas — sem faturamento**:
  - [ ] grep `-E "(R\$|fatura|faturamento|MRR|receita)"` no `.astro` retorna 0 menções relacionadas a empresas do {{NOME_OPERADOR_CURTO}} ({{PLATAFORMA_CURSOS}}, {{PRODUTO_PARCERIA}}, Projeto {{NOME_OPERADOR}}, {{EMPRESA_PRINCIPAL}})
  - [ ] Se houver prova social numérica, usa: usuários ativos, alunos, criadores, cases, tempo de mercado, marcas atendidas
  - [ ] Nenhuma menção a faturamento, MRR, lucro, ARR de {{PLATAFORMA_CURSOS}}, {{PRODUTO_PARCERIA}}, Projeto {{NOME_OPERADOR}}
  Falhar = REPROVAR.
...
Próximo passo: Agente Dev executar /ajustar-pagina com este arquivo.
```
...
- [ ] **Posicionamento de comunidade** (em produtos com mentor — mentoria, consultoria, eventos, cursos com presença do {{NOME_OPERADOR_CURTO}}):
  ...
  - [ ] O {{NOME_OPERADOR_CURTO}} (mentor/consultor) é claramente o protagonista de valor
...
- [ ] **Prova social — honesta, sobre o GUI, inequívoca**:
  - [ ] grep por métrica de usuários da plataforma retorna 0 ocorrências como métrica do {{NOME_OPERADOR_CURTO}} (pode aparecer descrevendo a {{PLATAFORMA_CURSOS}} em contexto claro: "plataforma com {{METRICA_PLATAFORMA}}+ usuários hospedando cursos")
