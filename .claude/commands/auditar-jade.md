...
|---|---|---|---|
| 1 | Nome operador | `\bgui\s?[áa]vila\b\|\b{{github_user}}\b` | Nome próprio do dono do squad |
| 2 | Paths absolutos | `/Users/[a-z]+` ou `/home/[a-z]+` | Username do sistema operacional |
| 3 | Nomes pessoais | `{{NOME_COFUNDADOR}}\|{{NOME_SUPORTE}}\|{{NOME_PARCEIRO_PLATAFORMA}}\|\|{{NOME_BACKUP_ADMIN}}\|{{NOME_CLIENTE_EXEMPLO}}\|{{email_cliente_exemplo}}` | Parceiros, suporte, clientes |
| 4 | Empresas reais | `{{EMPRESA_COFUNDADA}}\|{{EMPRESA_HOLDING}}\|{{EMPRESA_NEGOCIO}}\|{{PRODUTO_APP}}` (case insensitive, não-placeholder) | Empresas do operador |
...
| 9 | Workspace IDs | `\b30978229\b` (e outros IDs ClickUp/Notazz) | Identificadores de contas externas |
| 10 | Skills só do operador | `consultar-nf\|publicar-{{PRODUTO_APP}}\|responder-{{NOME_SUPORTE_LOWER}}\|tweet-imagem\|analisar-fiscal` | Skills que não fazem sentido pro aluno |
...
# 1. Nome operador
CHECKS[1_nome_operador]=$(grep -rEi '\bgui\s?[áa]vila\b|\b{{github_user}}\b' . 2>/dev/null | grep -v '.git/' | wc -l | tr -d ' ')
...
CHECKS[3_nomes_pessoais]=$(grep -rE '\b({{NOME_COFUNDADOR}}|{{NOME_SUPORTE}}|{{NOME_PARCEIRO_PLATAFORMA}}||{{NOME_BACKUP_ADMIN}}|{{NOME_CLIENTE_EXEMPLO}}|{{email_cliente_exemplo}})\b' . 2>/dev/null | grep -v '.git/' | wc -l | tr -d ' ')

# 4. Empresas reais
CHECKS[4_empresas]=$(grep -rEi '\b({{slug_produto_parceiro}}|fatorial|magica online|magica_online|gimmick)\b' . 2>/dev/null | grep -v '{{' | grep -v '.git/' | wc -l | tr -d ' ')
...
CHECKS[10_skills_operador]=$(grep -rE 'consultar-nf|publicar-gimmick|responder-lohan|tweet-imagem|analisar-fiscal' . 2>/dev/null | grep -v '.git/' | wc -l | tr -d ' ')
...
# Gerar relatório
{
  ...
  if [ "$TOTAL" != "0" ]; then
    echo '```'
    grep -rEi '\bgui\s?[áa]vila\b|/Users/[a-z]+|{{NOME_COFUNDADOR}}|{{NOME_SUPORTE}}|\bensinio\b|\bfatorial\b|\bgimmick\b|\b86[a-z0-9]{7,8}\b|armavita|consultar-nf|publicar-gimmick|responder-lohan|\b(PRD|business-rules|database|integrations)\.md\b' . 2>/dev/null | grep -v '.git/' | grep -v '{{' | head -20
    echo '```'
  else
...
- **2026-05-18:** Skill criada após red team encontrar 78 vazamentos em primeira publicação do `{{GITHUB_USER}}/jade` (PRD, paths absolutos, {{NOME_COFUNDADOR}}, ClickUp IDs, memórias `feedback_*.md`, MCP `armavita`, etc). Decisão {{NOME_OPERADOR}}: "Eu quero que você crie uma skill pra eu rodar ela de tempos em tempos pra garantir que é uma auditoria completa."
