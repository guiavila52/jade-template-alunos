...
| Critério | Pergunta |
|----------|----------|
| **Quality Score** | Output precisou de retrabalho? Teve erros? Manteve o tom do {{NOME_OPERADOR_CURTO}}? |
...
- Promoção sempre exige critério atingido + confirmação do {{NOME_OPERADOR_CURTO}}
- Desativação exige confirmação do {{NOME_OPERADOR_CURTO}} antes de remover do TEAM.md
...
[ 3. Decisão por agente ] → @jade
   ┌──────────────────────────────────────┐
   ↓ 🏆 Promover (critério + consistência — exige OK {{NOME_OPERADOR_CURTO}})
   ↓ 📊 Manter
   ↓ 📉 Rebaixar (2 erros graves ou 2 retrabalhos)
   ↓ ❌ Desativar (4+ semanas sem uso — exige OK {{NOME_OPERADOR_CURTO}})
        ↓
[ 4. Registrar ] → @jade
   ...
   - diario/[data].md (se houve decisão importante)
        ↓
[ 5. Entregar relatório ao {{NOME_OPERADOR_CURTO}} ] → @jade
