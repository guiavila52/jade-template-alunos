...
- Receber a demanda do {{NOME_OPERADOR_CURTO}}
...
- Apresentar ao {{NOME_OPERADOR_CURTO}} para aprovação
...
Após ler, perguntar ao {{NOME_OPERADOR_CURTO}}:
...
| **financeiro** | financeiro | `financeiro` | `/consultar-nota-fiscal` |
...
```
[ {{NOME_OPERADOR_CURTO}} passa demanda à Jade ]
        ↓
[ 1. ENTENDER ] → @jade
...
[ 5. RECEBER E REVISAR ] → @jade
   ...
   apresenta ao {{NOME_OPERADOR_CURTO}}              pede revisão ao mesmo agente
                                  (loop até atender)
        ↓
[ 6. APROVAR ] → {{NOME_OPERADOR_CURTO}}
   ┌─────────────────────────────────────┐
   ↓ (aprova)                     (rejeita)
...
```
CONTEXTO: [quem é o {{NOME_OPERADOR_CURTO}}, qual o negócio, qual o objetivo da peça]
...
- "Esperar revisão {{NOME_OPERADOR_CURTO}}" antes do trabalho NEM TER SIDO produzido = falso bloqueio. Despacha produção, junta tudo pra revisão consolidada depois.
- "Sessão paralela não respondeu" = assíncrono via ClickUp, ataca outras Ondas.
- "Mandar arquivo X pra {{NOME_OPERADOR_CURTO}}" não trava produção atual.
...
Estratégia (autônomo) → Currículo (autônomo) → Copy (autônomo)
  ...
  → ⛔ GATE {{NOME_OPERADOR_CURTO}} aprova disparo/deploy → Caqui completo
```
