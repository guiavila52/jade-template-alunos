...
- Gui pergunta "o que tá pendente?"
...
| `tags` | array (ex: `["imersao-14-05"]`) | todas |
...
- HTTP 401 → token inválido/expirado → reportar pro Gui pra rotacionar key
...
3. Sinalizar pendências bloqueadas por input do Gui (tag `aprovacao-gui`)
4. Sugerir próximas 3 a atacar autonomamente

## Aprendizado + pendência (Regra §5)

- Se Gui apontar inconsistência (task no painel mas não veio na lista): registrar em `squads/gestao/aprendizados.md` + criar pendência de fix de filtro
