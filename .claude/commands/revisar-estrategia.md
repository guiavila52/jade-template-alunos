...
>
> Acionada automaticamente após `/escrever-estrategia` salvar o output, ou diretamente pelo {{NOME_OPERADOR_CURTO}} quando ele quiser que a Jade revise uma estratégia anterior.
...
- [ ] Banco de histórias do {{NOME_OPERADOR_CURTO}} referenciado (≥1 história do `banco-de-historias.md`)
...
#### Prova social
- [ ] Prova social escolhida está na lista permitida (sem {{METRICA_PLATAFORMA}}/35+/3 continentes/faturamento)
...
- [ ] Sem ambiguidade em "{{METRICA_PLATAFORMA}} usuários {{PLATAFORMA_CURSOS}}" (sempre com contexto: plataforma, não público pessoal)
- [ ] Sem mencionar empresas de terceiros sem autorização
...
- [ ] Quando perguntar pro {{NOME_OPERADOR_CURTO}} antes de redigir está descrito
- [ ] Briefing é executável dentro de Light Copy (sem reinventar tom)

#### Decisões pendentes (SEÇÃO 11)
- [ ] Tudo que SÓ o {{NOME_OPERADOR_CURTO}} pode dar está listado
- [ ] Estrategista NÃO escolheu pelo {{NOME_OPERADOR_CURTO}} (sem decisão silenciosa)
...
- Notificar {{NOME_OPERADOR_CURTO}}: "Estratégia [slug] aprovada. Despachei [skill consequente]."
...
2. Prova social inflada (faturamento, "{{METRICA_PLATAFORMA}} usuários" como público pessoal, "35+ empresas")
...
5. Inventar contexto do {{NOME_OPERADOR_CURTO}} sem fonte no segundo-cerebro
...
## Próximo passo
- [skill consequente despachada | devolução pro estrategista | aguardando input do {{NOME_OPERADOR_CURTO}}]
```
