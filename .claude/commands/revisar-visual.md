...
- **contexto:** produto/campanha relacionada ({{EMPRESA_COFUNDADA}}, {{EMPRESA_NEGOCIO}}, Projeto Gui, etc)
...
- Paleta errada ({{EMPRESA_COFUNDADA}} azul em LP do Projeto Gui)
...
- Tom visual alinhado com Gui (profissional-acessível, não corporativo-frio nem casual-meme)

**Source of truth:**
- `segundo-cerebro/01-identidade/` — tom visual Gui
...
- Visual não-alinhado com posicionamento (Gui = especialista IA, não influencer fitness)
...
- Imagens não cortam conteúdo crítico (rosto do Gui cortado)
...
### Regra Inviolável #14
Toda correção do Gui vira aprendizado permanente:
...
**Nota:** Páginas Astro = revisor-codigo-pagina é responsável. Revisor-visual só entra se migração pixel-perfect OU se Gui pedir explicitamente.
...
/revisar-visual --tipo=carrossel --path=/workspace/output/conteudo/carrossel-exemplo-aprovado.png --contexto="Projeto Gui"
...
- `feedback_metricas_publicas_gui.md` — faturamento nunca exposto
