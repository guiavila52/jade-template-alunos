...
2. **Confirmar com {{NOME_OPERADOR_CURTO}}** que aba foi fechada (uma confirmação explícita por execução).
...
Reportar pro {{NOME_OPERADOR_CURTO}} antes do `mv`:
- Linhas antes/depois
...
**PARAR. Aguardar "aprovado" explícito do {{NOME_OPERADOR_CURTO}}.** Não executar `mv` sem aval.
