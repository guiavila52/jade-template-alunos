...
```
/revisar-codigo-pagina $CLAUDE_PROJECT_DIR Astro {{NOME_OPERADOR}}/src/pages/[slug]/index.astro
```
...
- [ ] **TESTE FUNCIONAL DE SCROLL — não basta inspecionar markup (Regra #19, Tarefa #131, 06/05/2026):**
  ...
  - **Falhar = REPROVAR.** Inspecionar markup NÃO substitui scroll real. Citação {{OPERADOR}}: *"Bati scroll e o site ficou todo tremendo. Como é que isso passou na revisão? Uma coisa tão óbvia."*
...
- [ ] **Fidelidade à referência visual aprovada (Regra #19, 06/05/2026):**
  ...
  - **Falhar = REPROVAR imediatamente.** Reinterpretar layout aprovado é desfazer a aprovação do {{OPERADOR}}. Histórico: `/squad-time-ia` v1 (06/05/2026) reprovada por trocar rail horizontal + Jade sticky por grid 2x2 + Jade central.
...
- [ ] **Auditoria de fontes obrigatória** se a página é uma migração pixel perfect:
  - [ ] Rodei `cd "Sites Astro {{OPERADOR}} Avila" && node scripts/audit-fonts.mjs <urlOriginal> http://localhost:4321/[slug]`
...
- [ ] **Tipografia hero/h1/h2/h3 grandes em Syne (Regra #19, 06/05/2026) — REPROVAR se falhar:**
  ...
  - [ ] Hero foi visualmente comparado com `/reverso` (referência aprovada pelo {{OPERADOR}}) e não está achatado/distorcido?
...
- [ ] **Hiperlinks padronizados:** toda menção a {{EMPRESA_COFUNDADA}}, {{EMPRESA_NEGOCIO}}, YouTube, ClickUp 8x, Automações, Reverso, Imersão, Mentoria, Consultoria está com hiperlink seguindo padrão `https://{{handle}}.com/[slug]` (ver `DESIGN-SYSTEM.md` seção Hiperlinks). Reprovar se houver menção textual sem link.
- [ ] **Hiperlinks INLINE — link na palavra, NUNCA URL como texto (Regra #19, tarefa #110 — 06/05/2026):**
  - [ ] grep `{{handle}}\.com` no `.astro` retorna 0 ocorrências em **texto puro/visível** (fora de `href=`, fora de comentários `//`, fora de strings JS de mapping de slug)
  - [ ] Toda menção visível é `<a href="https://{{handle}}.com/[slug]" class="link-inline">palavra</a>` (palavra-âncora, não URL)
  - [ ] **Sem URLs entre parênteses como texto pra copiar/colar** (ex: ❌ "consultoria ({{handle}}.com/consultoria)" → ✅ "consultoria" como anchor)
  - [ ] Slugs respeitam padrão canônico (`magicaonline`, `manychat`, `clickup`, `clickup8x`, `level`, `automacoes`, `reverso`, `youtube`, `mentoria`, `consultoria`, `{{empresa_cofundada}}` — ver `project_hiperlinks_padrao.md`)
  - [ ] Comando rápido de auditoria: `grep -nE '\({{handle}}\.com|{{handle}}\.com\)' src/pages/[slug]/index.astro` deve retornar **0 hits** em texto visível.
  Falhar = REPROVAR. **Histórico:** /mentoria FAQ v2 (06/05/2026) tinha "consultoria ({{handle}}.com/consultoria)" — {{OPERADOR}} rejeitou. Citação: "não faz sentido botar entre parênteses como texto que a pessoa vai ter que copiar e colar."
...
- [ ] **Iframe — altura SEM corte E SEM excesso (Regra #19, tarefa #109 06/05/2026):**
  ...
  - [ ] Falhar = REPROVAR. Bug histórico: /mentoria 06/05/2026 com 1500-1600px de min-height gerou buraco gigante até FAQ — {{OPERADOR}} rejeitou.
- [ ] **Auditoria sistêmica de iframe — TODAS as páginas com iframe (Regra #19, tarefa #116 06/05/2026):**
  ...
  - [ ] Se uma regra de iframe foi adicionada por feedback do {{OPERADOR}}, RETROFITAR em TODAS as páginas com o mesmo padrão (ex: forms GHL/52fatorial), não só a que originou o feedback.
  ...
  ⚠️ **Bug histórico (06/05/2026, tarefa #116):** Regra #109 sobre iframe gap (form_embed.js + min-height modesto) foi aplicada só em /mentoria. /consultoria continuou com listener custom + min-height 1600/1450px e gap até a próxima seção. {{OPERADOR}} detectou. Falha de cobertura — retrofit não foi sistêmico. Citação {{OPERADOR}}: "Novamente aquele problema do buraco. (...) deveria ter sido corrigido por quem fez a revisão dessa página. Porque é só passar pelo layout e ver que tem buraco aí."
...
- [ ] **Sliders — modo correto pra contexto (Regra #19, decisão {{OPERADOR}} 06/05/2026):**
...
- [ ] **Slider — TESTE FUNCIONAL Playwright (Tarefa #103, GATE INVIOLÁVEL):**
  ...
  **Falhar em qualquer ponto = REPROVAR mesmo se markup parecer correto.**
  ...
  - `<script is:inline>{`...`}</script>` sintaxe quebrada — emite template literal como statement, código nunca executa (bug encontrado em #103, afetava `inscricao-aula-{{handle}}-{{empresa_cofundada}}` e `oferta-irresistivel-{{empresa_cofundada}}`)
...
- [ ] **Cursor affordance no slider (Tarefa #139, Regra #19, 06/05/2026):**
  ...
  **Histórico:** {{OPERADOR}} rejeitou 07/05/2026 (#139) sliders /reverso por falta de affordance. Causa-raiz: `.testi-card { cursor: default }` sobrescrevia `cursor: grab` do `.testi-track`. Solução canônica: filhos do track usam `cursor: inherit`, regras dentro de `@media (hover: hover) and (pointer: fine)`, JS usa classList.


  **Bug histórico:** 3 rejeições do {{OPERADOR}} em sliders sem drag funcional. Sempre que o revisor confiou em grep de markup ("vejo o JS no arquivo, parece OK"), passou batido. Esse teste funcional é o GATE — se passar aqui, drag está garantido em produção.

  **Citação {{OPERADOR}} (rail, 06/05/2026):** "Tem que ser fluido, leve. Eu tenho que pegar card e arrastar pra lado, pro outro, de jeito gostoso, de jeito macio."
...
- [ ] **Auditoria de TODO container horizontal scrollável (Regra #19, Tarefa #146 — 07/05/2026):**
  ...
  **Histórico:** {{OPERADOR}} rejeitou `/automacoes` em 07/05/2026 (#146) — 2 sliders de módulos (40 cards 232x309 cada, "MÓDULO 1 Primeiros passos / MÓDULO 2 Ferramentas / MÓDULO 3 Inteligência artificial" + 17 outros) congelados em produção porque o snapshot Framer não trouxe o runtime React/framer-motion. Citação: *"Esses slides dos módulos não estão de acordo com o que a gente já combinou. Todo slider tem que estar em movimento, sutil movimento, leve e contínuo, e quando colocar o cursor do mouse, tem que deixar a pessoa arrastar pra lado e pro outro. Como é que esse furo passou batido?"*. Causa-raiz: revisor verificou markup mas não detectou que não havia handler de drag/auto-scroll porque o pai era `<section overflow:hidden>` Framer-style (sem `overflow-x:auto`) — esse novo gate cobre exatamente esse caso.

- [ ] **Assets externos clonados em `public/` (Regra Inviolável #19, hotfix #86 — 06/05/2026):** toda imagem, vídeo, fonte, ícone, CSS ou JS referenciado pelo `.astro` (seja por URL absoluta `https://sites.{{handle}}.com/...` ou path relativo `/[slug]/img/...`) tem que existir em `Páginas Astro {{NOME_OPERADOR}}/public/[caminho-relativo]`. Não é opcional.
  - [ ] Rodei: `grep -hoE 'src="https://sites\.{{handle}}\.com/[^"]+\.(jpg|jpeg|png|webp|svg|gif|mov|mp4|webm|woff|woff2|ttf|otf|css|js|ico)"' src/pages/[slug]/index.astro | sort -u` → tenho a lista de URLs absolutas.
  ...
  - [ ] **Validação cenário "Vercel novo":** rodei script Playwright que bloqueia `sites.{{handle}}.com` e contei `imgs.filter(i => !i.complete || i.naturalWidth === 0).length` → tem que ser **0**. Se for > 0, são exatamente os assets que vão quebrar em produção num projeto Vercel novo.

  **Falhar em qualquer item = REPROVAR a página.** Migração SEM assets clonados não é entrega completa. Diff visual padrão MASCARA o problema (ambos os lados puxam da mesma CDN antiga e dão "PASS" enganoso). Hotfix histórico: #82 (clickup8x — vídeo .mov 36MB faltava, detectado pelo {{OPERADOR}}) + #86 (sistêmico — 46 assets em 4 páginas). Em produção via CDN antiga (`sites.{{handle}}.com`) carrega; em qualquer projeto Vercel novo OU se o domínio antigo cair, tudo 404.
...
- [ ] **Slugs WordPress NÃO são páginas a migrar (Regra Inviolável #19, hotfix #98 — 06/05/2026):** os slugs públicos `{{handle}}.com/[slug]` (`/magicaonline`, `/manychat`, `/clickup`, `/level`, `/automacoes`, `/reverso`, `/youtube`, `/{{empresa_cofundada}}`) são **redirects do WordPress**, NÃO páginas próprias do Astro. Exceção: `/clickup8x` É página Astro real.
  ...
  - [ ] Hiperlinks pra empresas/parceiros usam `https://{{handle}}.com/[slug]` com `target="_blank" rel="noopener"` (ex: `{{handle}}.com/magicaonline`, `{{handle}}.com/clickup`) — NÃO `sites.{{handle}}.com/[slug]` (esse domínio é só o sistema Astro do squad).
  - [ ] Lista canônica de slugs WP: `magicaonline`, `manychat`, `clickup`, `clickup8x` (← ESSE É PÁGINA), `level`, `automacoes`, `reverso`, `youtube`, `{{empresa_cofundada}}`, `consultoria`, `palestras`, `shortcuts`, `percepcao`.
  - [ ] Padrão de hiperlink pra produto/parceiro do {{OPERADOR}} é SEMPRE via `{{handle}}.com/[slug]` (memória `project_hiperlinks_padrao.md` + `project_redirects_wordpress.md`), porque o roteamento fica centralizado no WordPress.

  **Falhar em qualquer item = REPROVAR a página.** Slug WP tratado como rota Astro vai 404 em produção. Hiperlink pra produto via `sites.{{handle}}.com/[slug]` quebra a estratégia de centralização de redirects ({{OPERADOR}} muda destino no WP sem precisar redeployar páginas).
...
```bash
HTML=$(curl -s https://sites.{{handle}}.com/[slug])
...
Toda página em `sites.{{handle}}.com/*` DEVE servir o mesmo favicon canônico do squad:
...
REF=$(curl -s https://sites.{{handle}}.com/ | grep -oE '<link[^>]*rel=["\']?icon["\']?[^>]*>' | head -1)
TARGET=$(curl -s https://sites.{{handle}}.com/[slug] | grep -oE '<link[^>]*rel=["\']?icon["\']?[^>]*>' | head -1)
...
```bash
curl -sI "https://sites.{{handle}}.com/images/favicon-gui.ico" | head -1  # esperado: HTTP/2 200
curl -sI "https://sites.{{handle}}.com/images/favicon-gui.png" | head -1  # esperado: HTTP/2 200
```
...
```bash
node scripts/test-rail-smoothness-165.mjs https://sites.{{handle}}.com/<pagina>
```
...
**Caveat headless:** drag programático pode não disparar momentum runtime mesmo com código presente (PointerCapture rejeita events sintéticos). Por isso o teste de runtime pós-mouseup é WARN (informacional), não FAIL — o teste estrutural (código no bundle) é o gate. Drag manual real precisa ser testado pelo {{OPERADOR}} antes de marcar entregue.
...
Histórico: tarefa #182 abriu exceção indevida pra "ano semântico" em /natal (`<span class="year-accent">2027</span>` com Cormorant italic). {{OPERADOR}} reprovou imediatamente em #188 (07/05/2026): "nunca nunca nunca usar essa fonte para números — fica distorcido demais".
...
# Após build:
npx astro build
grep -c 'Copyright 2026 {{OPERADOR}}' dist/$SLUG/index.html  # esperado: 1 (não 2)
```

Se algum grep der ≥ 1 no source OU ≥ 2 no HTML built → **REPROVAR**.

**Bug histórico:** /mentoria redesign 2026-05-14 — passou batido pelo revisor código + revisor visual + bug-hunter. {{OPERADOR}} detectou. Esse item é GATE permanente daqui pra frente.
