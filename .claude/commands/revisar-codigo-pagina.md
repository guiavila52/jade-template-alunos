...
```
/revisar-codigo-pagina /Users/{{SEU_USUARIO}}/Documents/Projetos IA {{NOME_OPERADOR}}/Páginas Astro {{NOME_OPERADOR}}/src/pages/[slug]/index.astro
```
...
- [ ] **Auditoria de fontes obrigatória** se a página é uma migração pixel perfect:
  - [ ] Rodei `cd "Sites Astro {{NOME_OPERADOR}} Avila" && node scripts/audit-fonts.mjs <urlOriginal> http://localhost:4321/[slug]`
...
- [ ] **Hiperlinks INLINE — link na palavra, NUNCA URL como texto (Regra #19, tarefa #110 — 06/05/2026):**
  - [ ] grep `{{github_user}}\.com` no `.astro` retorna 0 ocorrências em **texto puro/visível** (fora de `href=`, fora de comentários `//`, fora de strings JS de mapping de slug)
  ...
  - [ ] Slugs respeitam padrão canônico (`{{SLUG_PRODUTO_PRINCIPAL}}`, `manychat`, `clickup`, `clickup8x`, `level`, `automacoes`, `reverso`, `youtube`, `mentoria`, `consultoria`, `{{slug_produto_parceiro}}` — ver `project_hiperlinks_padrao.md`)
  - [ ] Comando rápido de auditoria: `grep -nE '\({{github_user}}\.com|{{github_user}}\.com\)' src/pages/[slug]/index.astro` deve retornar **0 hits** em texto visível.
...
- [ ] **Auditoria sistêmica de iframe — TODAS as páginas com iframe (Regra #19, tarefa #116 06/05/2026):**
  ...
  - [ ] Se uma regra de iframe foi adicionada por feedback do {{NOME_OPERADOR}}, RETROFITAR em TODAS as páginas com o mesmo padrão (ex: forms GHL/{{empresa_holding_slug}}), não só a que originou o feedback.
...
- [ ] **Slider — TESTE FUNCIONAL Playwright (Tarefa #103, GATE INVIOLÁVEL):**
  ...
  **Falhar em qualquer ponto = REPROVAR mesmo se markup parecer correto.**
  ...
  - `<script is:inline>{`...`}</script>` sintaxe quebrada — emite template literal como statement, código nunca executa (bug encontrado em #103, afetava `inscricao-aula-{{NOME_OPERADOR_SLUG}}-{{slug_produto_parceiro}}` e `oferta-irresistivel-{{slug_produto_parceiro}}`)
...
- [ ] **Assets externos clonados em `public/` (Regra Inviolável #19, hotfix #86 — 06/05/2026):** toda imagem, vídeo, fonte, ícone, CSS ou JS referenciado pelo `.astro` (seja por URL absoluta `https://sites.{{DOMINIO}}/...` ou path relativo `/[slug]/img/...`) tem que existir em `Páginas Astro {{NOME_OPERADOR}}/public/[caminho-relativo]`. Não é opcional.
  - [ ] Rodei: `grep -hoE 'src="https://sites\.{{github_user}}\.com/[^"]+\.(jpg|jpeg|png|webp|svg|gif|mov|mp4|webm|woff|woff2|ttf|otf|css|js|ico)"' src/pages/[slug]/index.astro | sort -u` → tenho a lista de URLs absolutas.
...
- [ ] **Slugs WordPress NÃO são páginas a migrar (Regra Inviolável #19, hotfix #98 — 06/05/2026):** os slugs públicos `{{DOMINIO}}/[slug]` (`/{{SLUG_PRODUTO_PRINCIPAL}}`, `/manychat`, `/clickup`, `/level`, `/automacoes`, `/reverso`, `/youtube`, `/{{slug_produto_parceiro}}`) são **redirects do WordPress**, NÃO páginas próprias do Astro. Exceção: `/clickup8x` É página Astro real.
  - [ ] Verifiquei: nenhum link tipo `<a href="/{{SLUG_PRODUTO_PRINCIPAL}}">` está sendo tratado como rota Astro (esses slugs são redirects públicos do WordPress, não páginas próprias).
  - [ ] Hiperlinks pra empresas/parceiros usam `https://{{DOMINIO}}/[slug]` com `target="_blank" rel="noopener"` (ex: `{{DOMINIO}}/{{SLUG_PRODUTO_PRINCIPAL}}`, `{{DOMINIO}}/clickup`) — NÃO `sites.{{DOMINIO}}/[slug]` (esse domínio é só o sistema Astro do squad).
  - [ ] Lista canônica de slugs WP: `{{SLUG_PRODUTO_PRINCIPAL}}`, `manychat`, `clickup`, `clickup8x` (← ESSE É PÁGINA), `level`, `automacoes`, `reverso`, `youtube`, `{{slug_produto_parceiro}}`, `consultoria`, `palestras`, `shortcuts`, `percepcao`.
...
Histórico: tarefa #182 abriu exceção indevida pra "ano semântico" em /natal (`<span class="year-accent">2027</span>` com Cormorant italic). {{NOME_OPERADOR}} reprovou imediatamente em #188 (07/05/2026): "nunca nunca nunca usar essa fonte para números — fica distorcido demais".
...
# Após build:
npx astro build
grep -c 'Copyright 2026 {{NOME_OPERADOR}}' dist/$SLUG/index.html  # esperado: 1 (não 2)
```

Se algum grep der ≥ 1 no source OU ≥ 2 no HTML built → **REPROVAR**.

**Bug histórico:** /mentoria redesign 2026-05-14 — passou batido pelo revisor código + revisor visual + bug-hunter. {{NOME_OPERADOR}} detectou. Esse item é GATE permanente daqui pra frente.
