...
4. **{{GTM_CONTAINER_ID}} presente** — 2+ ocorrências (head + noscript body)
5. **Favicon canônico** — 
...
```
/revisar-codigo-pagina /Users/{{USERNAME_MAC}}/Documents/Projetos IA {{NOME_OPERADOR}}/Páginas Astro {{NOME_OPERADOR}}/src/pages/[slug]/index.astro
```
...
- [ ] **TESTE FUNCIONAL DE SCROLL — não basta inspecionar markup (Regra #19, Tarefa #131, 06/05/2026):**
  ...
  - [ ] Bug HISTÓRICO: `/sua-pagina` 06/05/2026 ficou tremendo no scroll vertical depois do collapse listener (#117). Revisor passou batido — não pode repetir.
...
- [ ] **Fidelidade à referência visual aprovada (Regra #19, 06/05/2026):**
  ...
  - **Falhar = REPROVAR imediatamente.** Reinterpretar layout aprovado é desfazer a aprovação do {{NOME_OPERADOR_CURTO}}.
...
- [ ] **Auditoria de fontes obrigatória** se a página é uma migração pixel perfect:
  ...
  - **Falhar = REPROVAR a migração mesmo se diff visual passou.** Pixel diff visual NÃO substitui auditoria de font-family.
...
- [ ] **Iframe — altura SEM corte E SEM excesso (Regra #19, tarefa #109 06/05/2026):**
  - [ ] Página usa `<script src="https://link.{{CRM_FORM_HOST}}/js/form_embed.js">` (oficial GHL) — NÃO listener custom postMessage
  ...
  - [ ] Falhar = REPROVAR. Bug histórico: /sua-pagina 06/05/2026 com 1500-1600px de min-height gerou buraco gigante até FAQ — {{NOME_OPERADOR_CURTO}} rejeitou.
- [ ] **Auditoria sistêmica de iframe — TODAS as páginas com iframe (Regra #19, tarefa #116 06/05/2026):**
  ...
  - [ ] Comando: `grep -rln "<iframe" "Páginas Astro {{NOME_OPERADOR}}/src/pages/" --include="*.astro"` → para cada página da lista, validar min-height vs altura real do form via Playwright.
  ...
  - [ ] Se uma regra de iframe foi adicionada por feedback do {{NOME_OPERADOR_CURTO}}, RETROFITAR em TODAS as páginas com o mesmo padrão (ex: forms GHL/{{empresa_principal}}), não só a que originou o feedback.
  ...
  ⚠️ **Bug histórico (06/05/2026, tarefa #116):** Regra #109 sobre iframe gap (form_embed.js + min-height modesto) foi aplicada só em /sua-pagina. /sua-pagina-2 continuou com listener custom + min-height 1600/1450px e gap até a próxima seção. {{NOME_OPERADOR_CURTO}} detectou. Falha de cobertura — retrofit não foi sistêmico. Citação {{NOME_OPERADOR_CURTO}}: "Novamente aquele problema do buraco. (...) deveria ter sido corrigido por quem fez a revisão dessa página. Porque é só passar pelo layout e ver que tem buraco aí."
...
- [ ] **Sliders — modo correto pra contexto (Regra #19, decisão {{NOME_OPERADOR_CURTO}} 06/05/2026):**
...
- [ ] **Sliders modo `rail` — drag macio com momentum (TESTE MANUAL OBRIGATÓRIO):**
  ...
  Falhar em qualquer um destes = **REPROVAR** a página. Sempre usar o componente canônico `<Slider mode="rail">` ou `<Slider>` (marquee default) — rail inline custom (`<div data-rail>`) está BANIDO. Para logos: `<LogoSlider>` (wrapper de marquee).
...
- [ ] **Cursor affordance no slider (Tarefa #139, Regra #19, 06/05/2026):**
  ...
  **Por quê:** `.testi-card { cursor: default }` sobrescreve `cursor: grab` do `.testi-track`. Solução canônica: filhos do track usam `cursor: inherit`, regras dentro de `@media (hover: hover) and (pointer: fine)`, JS usa classList.


  **Bug histórico:** 3 rejeições do {{NOME_OPERADOR_CURTO}} em sliders sem drag funcional. Sempre que o revisor confiou em grep de markup ("vejo o JS no arquivo, parece OK"), passou batido. Esse teste funcional é o GATE — se passar aqui, drag está garantido em produção.

  **Citação {{NOME_OPERADOR_CURTO}} (rail, 06/05/2026):** "Tem que ser fluido, leve. Eu tenho que pegar card e arrastar pra lado, pro outro, de jeito gostoso, de jeito macio."
...
- [ ] **Auditoria de TODO container horizontal scrollável (Regra #19, Tarefa #146 — 07/05/2026):**
  ...
  const horizontalContainers = await page.evaluate(() => {
    ...
    return all.filter(el => {
      ...
      isCanonical: !!el.closest('[data-slider]'),
    }));
  });
  ```

  Para cada container detectado:
  - [ ] **`isCanonical === true`** (filho de `[data-slider]`) — OU é justificadamente um grid estático intencional (depoimentos 3-col, etc).
  ...
  **Por quê:** slider pode passar na inspeção de markup mas congelar em produção se o pai usar `overflow:hidden` Framer-style (sem `overflow-x:auto`) — revisor deve verificar handler de drag/auto-scroll, não só o markup.
...
- [ ] **Assets externos clonados em `public/` (Regra Inviolável #19, hotfix #86 — 06/05/2026):** toda imagem, vídeo, fonte, ícone, CSS ou JS referenciado pelo `.astro` (seja por URL absoluta `https://sites.{{DOMINIO}}/...` ou path relativo `/[slug]/img/...`) tem que existir em `Páginas Astro {{NOME_OPERADOR}}/public/[caminho-relativo]`. Não é opcional.
  - [ ] Rodei: `grep -hoE 'src="https://sites\.{{DOMINIO}}/[^"]+\.(jpg|jpeg|png|webp|svg|gif|mov|mp4|webm|woff|woff2|ttf|otf|css|js|ico)"' src/pages/[slug]/index.astro | sort -u` → tenho a lista de URLs absolutas.
...
### {{GTM_CONTAINER_ID}} — OBRIGATÓRIO no HTML servido
...
GTM_COUNT=$(echo "$HTML" | grep -o '{{GTM_CONTAINER_ID}}'              | wc -l)   # >= 2
...
Páginas com snapshot externo (Framer/Webflow): confirmar que o GTM presente é `{{GTM_CONTAINER_ID}}` (nosso), não o GTM da plataforma original. Duplicação de noscript {{GTM_CONTAINER_ID}} (Base.astro + snapshot legacy) é tolerável (GTM ignora cargas duplicadas com mesmo ID).
...
```bash
curl -sI "https://sites.{{DOMINIO}}/images/favicon.ico" | head -1  # esperado: HTTP/2 200
curl -sI "https://sites.{{DOMINIO}}/images/favicon.png" | head -1  # esperado: HTTP/2 200
```
...
**Caveat headless:** drag programático pode não disparar momentum runtime mesmo com código presente (PointerCapture rejeita events sintéticos). Por isso o teste de runtime pós-mouseup é WARN (informacional), não FAIL — o teste estrutural (código no bundle) é o gate. Drag manual real precisa ser testado pelo {{NOME_OPERADOR_CURTO}} antes de marcar entregue.
...
**Regra inviolável:** fonte display (Cormorant, Syne, Fraunces) é proibida em números — fica distorcido demais. Usar sempre Inter Tight tabular-nums ou JetBrains Mono.
...
echo "$HTML" | grep -oE '<[^>]*class="[^"]*(year|cupom|numero|preco|data|cormorant)[^"]*"[^>]*>[^<]*[0-9]+[^<]*</' | head -5
```
...
Fonte: tarefa #188 (07/05/2026) — fix de regressão visual em /[sua-pagina].
...
const heroBg = await page.evaluate(() => getComputedStyle(document.querySelector('.hero-exemplo')).backgroundImage);
if (heroBg === 'none') {
...
```bash
SLUG="<slug-da-pagina>"
grep -c '<Footer' src/pages/$SLUG/index.astro  # esperado: 0
grep -c '<Header' src/pages/$SLUG/index.astro  # esperado: 0

# Após build:
npx astro build
grep -c 'Copyright 2026 {{NOME_OPERADOR_CURTO}}' dist/$SLUG/index.html  # esperado: 1 (não 2)
```

Se algum grep der ≥ 1 no source OU ≥ 2 no HTML built → **REPROVAR**.

**Bug histórico:** /mentoria redesign 2026-05-14 — passou batido pelo revisor código + revisor visual + bug-hunter. {{NOME_OPERADOR_CURTO}} detectou. Esse item é GATE permanente daqui pra frente.