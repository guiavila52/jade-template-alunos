...
Renderiza newsletter em HTML fragment a partir de config JSON, seguindo **template canônico v5** (baseline aprovado pelo operador).
...
| Artefato | Path |
|---|---|
| HTML baseline (v5 aprovada) | `workspace/output/newsletter/exemplo-v5.html` |
...
| Config exemplo v5 (regression fixture) | `workspace/output/newsletter/exemplo-config.json` |
...
```json
{
  ...
  "corpo_secoes": [
    ...
    {"tipo": "video_embed", "youtube_id": "XXXXXXXXXXX", "gancho": "Gravei um vídeo curto."},
...
Estrutura (literal):
- Foto perfil 96×96 circular → `https://sites.{{DOMINIO}}/assets-perfil/foto-perfil-circular.png`
...
- 4 linhas (literal — capitalização exata):
  ...
  3. Autor do {{PRODUTO_PRINCIPAL}}, {{PRODUTO_ENTRADA}} e {{PRODUTO_ENTRADA_2}}
