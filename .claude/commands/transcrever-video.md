...
description: Transcreve video YouTube ou arquivo local em texto puro com timestamps e verificacao obrigatoria de completude.
type: skill
---
...
- {{NOME_OPERADOR_CURTO}} manda URL de YouTube ou path de arquivo de vídeo/áudio e quer o conteúdo em texto.
...
Exemplos:
- `/transcrever-video https://youtu.be/XXXXXXXXXXX`
...
- Após montar parágrafos, rodar limpezas:
  - Colapsar palavras adjacentes repetidas: `(\w+)( \1)+` → `\1` (case-insensitive).
  - Normalizar espaços.
...
Path: `workspace/output/transcricoes/YYYY-MM-DD-{slug}.md`
...
Mostrar ao {{NOME_OPERADOR_CURTO}}: path do arquivo, duração, último timestamp, diff, e SUCESSO/FALHA.
...
`workspace/output/transcricoes/YYYY-MM-DD-{slug}.md` — atualizar `mapa.md` da pasta a cada transcrição.
...
- {{NOME_OPERADOR_CURTO}} direto: `/transcrever-video URL` ou `/transcrever-video /path/file.mp4`
- Outras skills (futuras):
  ...
  - `/criar-carrossel-de-video` → atalho ponta-a-ponta — primeiro nó do fluxo é sempre `/transcrever-video`
...
[ ✅ SUCESSO                        [ ❌ FALHA
   ...
   - reporta JSON ao {{NOME_OPERADOR_CURTO}} ]             - se retry falhar:
                                        salva [INCOMPLETA].md
                                        + alerta destacado ]
        ↓                                       ↓
        └────────────┬──────────────────────────┘
