---
name: criar-pagina
description: Orquestra criacao ponta-a-ponta de pagina Astro com pipeline template-first canonico. Anti-improviso. Template (premium/clean/gimmick) + frontend-design oficial Anthropic + designer-ui (DESIGN.md) + estrategista + copy + dev extends template + revisor visual real + triple-check + publicar.
type: skill
---
...
**Stack alvo:** projeto `Paginas Astro {{NOME_OPERADOR}} Avila/` (Astro 6 + Tailwind v4 + 3 templates canonicos em `src/layouts/template-*.astro`).
...
1. 3 templates Astro disponiveis em `Paginas Astro {{NOME_OPERADOR}} Avila/src/layouts/template-{premium,clean,gimmick}.astro` (se faltar algum, despachar criacao em branch separada antes)
...
## Inputs (perguntar ao {{NOME_OPERADOR}} se faltar)
...
6. Tem video pra embedar? URL do canal {{NOME_OPERADOR}} Avila
...
```
[0] ESCOLHA TEMPLATE → Jade decide premium/clean/gimmick com justificativa
        |
        v
...
[2] BRIEFING ESTRATEGICO → estrategista (11 secoes)
        |
        v
[3] DESIGN.md + briefing VALIDADOS por revisor independente (NAO {{NOME_OPERADOR}})
        |   {{NOME_OPERADOR}} valida APENAS codigo rodando localhost (Step 9)
        v
[4] COPY → copywriter
        |
        v
...
[8] TRIPLE-CHECK PARALELO → copywriter (revisao copy contextual) + dev-frontend (code review) + analista-qa (12 itens funcional)
        |
        v
[9] PREVIEW VERCEL — PR + auto-deploy + URL pro {{NOME_OPERADOR}} (hook §4 libera quando REVISAO-APROVADO presente)
        |
        v
...
| gimmick | Produto SaaS energetico, features/pricing | gimmick.{{DOMINIO}} |

Jade escolhe + justifica em 1 frase. Apresenta pro {{NOME_OPERADOR}} em mensagem curta. Se {{NOME_OPERADOR}} discordar, ajusta.
...
Input:
- Template escolhido: {premium|clean|gimmick}
- Tipo pagina, audiencia, objetivo (do briefing Jade)
- DS correspondente em workspace/design-systems/{{github_user}}-{template}.md
...
Input: DESIGN.md da Step 1 + tipo pagina + audiencia + objetivo {{NOME_OPERADOR}}
...
### Step 3 — Validacao independente (NAO {{NOME_OPERADOR}})

{{NOME_OPERADOR}} NAO revisa DESIGN.md nem briefing em markdown (aprendizado 17/05: ele valida APENAS codigo rodando, ver memoria `feedback_gui_valida_no_codigo_nao_markdown.md`).
...
Se validacao interna OK, prosseguir DIRETAMENTE pra Step 4 sem pausar pra {{NOME_OPERADOR}}.

**Gate {{NOME_OPERADOR}} acontece APENAS no Step 9** (preview localhost depois de tudo codado + revisado). {{NOME_OPERADOR}} reporta correcoes → aprendizado §5 vai pro agente que produziu o output original.
...
Aplicar: Light Copy + tom {{NOME_OPERADOR}} + Headline canonica (do briefing) + sem vocabulario banido
""")
```
...
- Olhar screenshot inteiro como {{NOME_OPERADOR}} olharia
...
### Step 9 — Preview Vercel + link pro {{NOME_OPERADOR}}

```bash
cd ../Paginas Astro {{NOME_OPERADOR}} Avila/
...
Link preview pro {{NOME_OPERADOR}}:
- Hook `check-mostrar-pro-gui-sem-revisor.sh` libera se REVISAO-APROVADO recente presente
- Senao bloqueia (forca nova auditoria)

### Step 10 — Gate {{NOME_OPERADOR}} aprova → merge

{{NOME_OPERADOR}} valida visualmente preview Vercel. Se aprova: merge PR pra main → auto-deploy producao. Se rejeita: volta Step 4 ou 6 conforme escopo da mudanca.
...
- Mandar URL pro {{NOME_OPERADOR}} sem REVISAO-APROVADO
- Pular triple-check
- Pular Step 3 (Gate {{NOME_OPERADOR}} no mockup)
...
- DESIGN.md vivo (atualiza se {{NOME_OPERADOR}} mudar direcao)
- Pagina extends template (sem visual inventado)
- Build limpo
- Scripts validacao passam
- Triple-check 3/3 aprovado
- Preview Vercel APROVADO {{NOME_OPERADOR}}
- Merge main → producao
- Aprendizado §5 propagado se houve correcao do {{NOME_OPERADOR}} no caminho

---

## Histórico

- 2026-05-17 v2: refactor template-first apos incidente 8 iteracoes na /squad-time-ia-v2 (improviso de despacho). Aval {{NOME_OPERADOR}} registrado. Task ClickUp 86ahha462.
...
Criado 17/05/2026 (aprendizado §5 — {{NOME_OPERADOR}} pegou rodapé faltando + visual sem graça + logo invisível 4 vezes).
...
3. **Hook bloqueante** garante que nada vai pro {{NOME_OPERADOR}} sem REVISAO-APROVADO recente
