...
- {{NOME_OPERADOR}} mandou URL do YouTube → carrossel a partir da transcrição
- {{NOME_OPERADOR}} mandou tema/ângulo solto → estrategista define ângulo único antes da copy
...
└─ Tema vago/curto → PERGUNTA ao {{NOME_OPERADOR}} antes (ângulo, ICP, qual produto)
```
...
|---|---|---|
| **Jade → estrategista** | input bruto (URL/tema) + transcrição (se vídeo) + ICP do {{NOME_OPERADOR}} | ângulo único + narrativa + payoff + qtd lâminas + estrutura hook→desenvolvimento→CTA + tom + refs segundo-cerebro |
...
## Triple-check obrigatório (antes de entregar ao {{NOME_OPERADOR}})
...
- **Métricas públicas** — nunca expor faturamento (R$/MRR) das empresas (Regra `feedback_metricas_publicas_gui.md`); usar usuários, alunos, criadores
...
```
[ {{NOME_OPERADOR}} dispara /criar-carrossel <input> ]
        ↓
[ Jade detecta tipo de input ]
   ...
   │                         ↓
   └─ Tema vago → ⟶ PERGUNTA ao {{NOME_OPERADOR}}
                            ↓
...
[ Apresenta ao {{NOME_OPERADOR}} ]
   ...
   sumário: copy + thumbs dos PNGs + path
        ↓
   ⟶ FIM (aguarda aprovação final do {{NOME_OPERADOR}})
```

## Captura de aprendizado (Regra #19 — obrigatório após aprovação ou rejeição)

Quando {{NOME_OPERADOR}} aprovar/rejeitar, registrar em DOIS lugares:
...
### [título curto]
**Data:** YYYY-MM-DD | **Slug:** [slug]
**O que não funcionou:** [o que o {{NOME_OPERADOR}} rejeitou]
