...
grep -hoE 'https://sites\.{{DOMINIO}}/[a-z0-9_-]+/(img|assets|files|cdn|fonts|videos)/[^"\)]+\.(jpg|jpeg|png|webp|svg|gif|mov|mp4|webm|woff|woff2|ttf|otf|css|js|ico)' src/pages/[slug]/index.astro | sort -u

# 1b. Reforçar com grep que aceita espaço em nome de arquivo (logos com nomes "banco do brasil.png")
grep -hoE 'src="https://sites\.{{DOMINIO}}/[^"]+\.(jpg|jpeg|png|webp|svg|gif|mov|mp4|webm|woff|woff2|ttf|otf|css|js|ico)"' src/pages/[slug]/index.astro | sort -u
...
[1] Extração de conteúdo
    Se URL → WebFetch
    Se descrição → pedir copy ao {{NOME_OPERADOR_CURTO}}
...
[4] Atualizar workspace/output/paginas/mapa.md
    Linha "Outputs produzidos" + atualizar "Última atualização"
        │
        ▼
[5] Apresentar ao {{NOME_OPERADOR_CURTO}}
    Resumo do diagnóstico (3-5 bullets)
    Caminho do arquivo
    Pergunta: "implementar agora ou ajustar copy antes?"
        │
        ├── {{NOME_OPERADOR_CURTO}} pediu ajuste? ─ sim → revisar markdown e voltar a [5]
        │
        ▼ {{NOME_OPERADOR_CURTO}} aprovou
...
[9] Despachar /publicar-pagina (modo preview)
    Retorna: http://localhost:4321/[slug]
        │
        ▼
[10] Apresentar URL localhost ao {{NOME_OPERADOR_CURTO}}
        │
        ├── {{NOME_OPERADOR_CURTO}} pediu ajuste? ─ sim → voltar a [6]
        │
        ▼ {{NOME_OPERADOR_CURTO}} aprovou
[11] Despachar /publicar-pagina (modo produção)
     vercel --prod no projeto Páginas Astro {{NOME_OPERADOR}}/
        │
        ├── deploy falhou? ─ sim → reportar ao {{NOME_OPERADOR_CURTO}}, NÃO retentar
        │
        ▼ deploy OK
[12] Atualizar registros finais:
     ...
     - squads/dev/tarefas.md (status: publicado)
        │
        ▼
[13] Notificar {{NOME_OPERADOR_CURTO}}:
...
2. `segundo-cerebro/01-identidade/tom-de-voz.md` — para manter a voz do {{NOME_OPERADOR_CURTO}}
...
Exemplos:
- `/migrar-pagina https://sites.{{DOMINIO}}/sua-pagina`
...
Se for URL, use WebFetch para capturar o conteúdo. Se for descrição, peça ao {{NOME_OPERADOR_CURTO}} que cole o copy da página.
...
- [ ] mapa.md atualizado
- [ ] Aguarda aprovação do {{NOME_OPERADOR_CURTO}} antes de seguir para /ajustar-pagina
...
Após o OK do {{NOME_OPERADOR_CURTO}} no markdown, encadear automaticamente:
...
3. `/publicar-pagina` → preview localhost → OK do {{NOME_OPERADOR_CURTO}} → `vercel --prod`
...
> **Nota:** Sempre inspecione fontes e variáveis CSS após migração — overrides de `font-family` podem não ser capturados por diff visual se as fontes tiverem tamanhos similares.
...
- **Nunca subir direto para produção** — o output passa pelo `/revisar-codigo-pagina` e pelo OK do {{NOME_OPERADOR_CURTO}} no preview localhost.
