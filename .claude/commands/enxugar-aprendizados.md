...
- Sob demanda do {{NOME_OPERADOR_CURTO}} ("aprendizados tão grandes demais")
...
**Citação {{NOME_OPERADOR_CURTO}} (opcional):** "[curta, ≤1 linha]"
```
...
- Citações longas do {{NOME_OPERADOR_CURTO}} (>1 linha) — preservar só essência
...
- Citação curta do {{NOME_OPERADOR_CURTO}} (1 linha)
...
- Citações {{NOME_OPERADOR_CURTO}} curtas mantidas
...
- Aprendizado real (correção do {{NOME_OPERADOR_CURTO}}, novo padrão de curadoria) → `squads/dev/agentes/devops/aprendizados.md`
