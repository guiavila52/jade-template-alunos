...
- Sob demanda do {{NOME_OPERADOR}} ("aprendizados tão grandes demais")
...
**Citação {{NOME_OPERADOR}} (opcional):** "[curta, ≤1 linha]"
```
...
- Citações longas do {{NOME_OPERADOR}} (>1 linha) — preservar só essência
...
- Citação curta do {{NOME_OPERADOR}} (1 linha)
...
- Citações {{NOME_OPERADOR}} curtas mantidas
...
- Aprendizado real (correção do {{NOME_OPERADOR}}, novo padrão de curadoria) → `squads/dev/agentes/devops/aprendizados.md`
