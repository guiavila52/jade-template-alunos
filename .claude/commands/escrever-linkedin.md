...
Opcionalmente, o {{NOME_OPERADOR_CURTO}} pode indicar:
...
LinkedIn é o canal de **autoridade B2B** do {{NOME_OPERADOR_CURTO}}. Posts curtos e opinionados, 3-5x por semana. O público no LinkedIn é fundador de negócio digital — mais formal que Instagram, mas ainda direto e autêntico. Sem "marketês", sem lista genérica de dicas.
...
**4. Bastidores**
O que o {{NOME_OPERADOR_CURTO}} está construindo na prática. Decisões reais, números reais, erros reais.
...
O {{NOME_OPERADOR_CURTO}} escolhe qual publica.
...
- Emojis: com naturalidade, como o {{NOME_OPERADOR_CURTO}} usa. Nada excessivo.
...
## Integração {{PLATAFORMA_NEWSLETTER}} (API REST)

Ao finalizar o post, criar no {{PLATAFORMA_NEWSLETTER}} via API REST (Bearer `{{CHAVE_API_EXEMPLO}}`, endpoints em `segundo-cerebro/03-operacao/{{plataforma_newsletter}}-historico.md`):

1. `POST /api/content/linkedin` — registrar o post no pipeline do {{PLATAFORMA_NEWSLETTER}} com `status: "aprovacao"`.
2. Validar com `GET /api/content/linkedin/{id}` após criação.
3. O {{NOME_OPERADOR_CURTO}} aprova ou solicita ajuste diretamente no painel visual do {{PLATAFORMA_NEWSLETTER}}.

**Nota histórica [DEPRECATED 2026-05-14]:** antes existia plano de usar MCP server (`mcp__{{plataforma_newsletter}}__criar_conteudo` / `atualizar_status`). MCP descontinuado em 12/05/2026 (retorna 401). API REST direta é o caminho canônico.
...
- [ ] Tom bate com o {{NOME_OPERADOR_CURTO}} (direto, autêntico, sem "marketês")?
...
```
[ {{NOME_OPERADOR_CURTO}} passa transcrição de vídeo (ou URL → /transcrever-video) ]
        ↓
...
[ 6. Salvar output ] → @linkedin
   workspace/output/conteudo/escrever-linkedin/YYYY-MM-DD-[slug].md
        ↓
   ⟶ {{NOME_OPERADOR_CURTO}} escolhe qual versão publica
        ↓
[ 7. (Se {{PLATAFORMA_NEWSLETTER}} API REST disponível)
