...
## Integração {{Plataforma_Conteudo}} (API REST)

Ao finalizar o post, criar no {{Plataforma_Conteudo}} via API REST (Bearer `sk-sq-*`, endpoints em `segundo-cerebro/03-operacao/{{plataforma_conteudo}}-historico.md`):

1. `POST /api/content/linkedin` — registrar o post no pipeline do {{Plataforma_Conteudo}} com `status: "aprovacao"`.
2. Validar com `GET /api/content/linkedin/{id}` após criação.
3. O Gui aprova ou solicita ajuste diretamente no painel visual do {{Plataforma_Conteudo}}.
...
[ 6. Salvar output ] → @linkedin
   ...
   ⟶ Gui escolhe qual versão publica
        ↓
[ 7. (Se {{Plataforma_Conteudo}} API REST disponível)
