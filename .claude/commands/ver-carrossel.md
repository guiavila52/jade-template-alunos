...
Função: acessar carrosséis do Instagram por URL e extrair texto, estrutura e observações visuais — sem que o {{OPERADOR}} precise mandar prints manualmente.

⚠️ **Esta skill usa o Chrome MCP para navegar e tirar screenshots.**
**NUNCA usar Chrome MCP ou tirar screenshots sem o {{OPERADOR}} chamar `/ver-carrossel [URL]` explicitamente.**
...
Se chamado sem URL, pedir ao {{OPERADOR}} que informe o link do post.
...
- Se o Instagram pedir login, **parar imediatamente** e informar ao {{OPERADOR}}:
...
Use `mcp__Claude_in_Chrome__navigate` com a URL fornecida pelo {{OPERADOR}}.
...
**O que funciona bem:** [pontos que podem inspirar carrosséis do {{OPERADOR}}]
**O que evitar:** [pontos fracos ou fora do tom do {{OPERADOR}}]
```
...
Se o {{OPERADOR}} pedir para salvar a referência, criar arquivo em:
...
- **Nunca** navegar no Instagram ou tirar screenshots sem o {{OPERADOR}} chamar esta skill explicitamente.
- **Nunca** salvar imagens/screenshots localmente — só descrições em texto.
- Se o Instagram bloquear o acesso (login, CAPTCHA, rate limit), informar ao {{OPERADOR}} imediatamente e não tentar contornar.
...
```
[ {{OPERADOR}} chama /ver-carrossel <URL_instagram> ]
   ⚠️ skill SÓ roda quando {{OPERADOR}} chama explicitamente
        ↓
[ 1. Verificar Chrome + login Instagram ] → @ver-carrossel
   ...
   ┌─────────────────────────────────────┐
   ↓ (logado)                     (pediu login)
[ 2. Navegar até a URL              [ ABORTAR + avisar {{OPERADOR}}:
