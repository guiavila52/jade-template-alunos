...
description: Leva componente Astro aprovado pelo revisor ate producao: build local, preview localhost, aprovacao {{NOME_OPERADOR_CURTO}} e entao deploy.
type: skill
---
...
Função: levar um componente Astro aprovado pelo `/revisar-codigo-pagina` até produção, com checkpoint de aprovação visual do {{NOME_OPERADOR_CURTO}} no meio.

⚠️ **Nunca rodar `vercel --prod` antes do {{NOME_OPERADOR_CURTO}} aprovar o preview localhost.**
...
[5] Retornar URL para a Jade
    ...
    (Jade apresenta ao {{NOME_OPERADOR_CURTO}})
        │
        ▼
[6] Aguardar confirmação da Jade
    "{{NOME_OPERADOR_CURTO}} aprovou o preview" → segue
    "{{NOME_OPERADOR_CURTO}} pediu ajuste" → abortar deploy, parar dev server,
...
[10] Atualizar squads/dev/tarefas.md
     status: publicado | data | URL produção em Obs
        │
        ▼
[11] Atualizar workspace/output/paginas/mapa.md
     Adicionar/atualizar linha com URL de produção
        │
        ▼
[12] Atualizar workspace/processos/pipeline-paginas.md
...
```
/publicar-pagina /Users/{{USERNAME_MAC}}/Documents/Projetos IA {{NOME_OPERADOR}}/Páginas Astro {{NOME_OPERADOR}}/src/pages/[slug]/index.astro
```
...
```bash
cd "/Users/{{USERNAME_MAC}}/Documents/Projetos IA {{NOME_OPERADOR}}/Páginas Astro {{NOME_OPERADOR}}"
...
- **{{NOME_OPERADOR_CURTO}} rejeita o preview:** abortar pipeline. Status no `tarefas.md` volta para `rejeitado` com observação. Devolver ao Agente Dev com apontamentos do {{NOME_OPERADOR_CURTO}}.
...
- workspace/output/paginas/mapa.md
- workspace/processos/pipeline-paginas.md
```
...
Em `workspace/output/paginas/mapa.md`, adicionar linha na tabela "Outputs produzidos" (ou atualizar a existente) com a URL de produção.
...
1. **`paginas`** (squad-copy) — APROVA copy?
   ...
   - Métricas públicas: nunca faturamento (Regra `feedback_metricas_publicas.md`)
...
2. **`paginas-dev`** (squad-dev) — APROVA código?
   ...
   - {{GTM_CONTAINER_ID}} presente (Regra #147)
...
## Fallback automático CLI→git push (Regra pós-incidente 13/05/2026)

Skill canônica DEVE usar `scripts/deploy/publicar-pagina.sh` em vez de `vercel --prod` direto.

Razão: em 13/05/2026, `vercel --prod` travou infinitamente nesta sessão Claude Code ({{NOME_OPERADOR_CURTO}} passou ~1h pra reverter páginas pré-Fase E). Workaround: `git push origin main` → Vercel auto-deploy GitHub funcionou em 30-90s.
...
```
bash /Users/{{USERNAME_MAC}}/Documents/Projetos\ IA\ {{NOME_OPERADOR_CURTO}}\ Ávila/Squad\ Empresa\ {{NOME_OPERADOR_CURTO}}\ Ávila/scripts/deploy/publicar-pagina.sh "/path/to/repo"
```