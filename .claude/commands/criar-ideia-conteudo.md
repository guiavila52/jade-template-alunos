...
Cria ideia de conteúdo crua no **{{APP_PESSOAL}}** (pipeline editorial) **via API REST direta (curl)**. Não usa MCP — funciona em subagent headless e cron.

Análoga à `/criar-pendencia` (ClickUp), mas o destino é o backlog editorial do {{APP_PESSOAL}}.

> Histórico/setup completo: `segundo-cerebro/03-operacao/{{app_pessoal}}-historico.md` (seções youtube/vertical schemas, descobertos 16/05/2026).

## Quando invocar

- {{OPERADOR}} passa ideia de conteúdo nova ("quero gravar vídeo sobre X")
...
**Regra §1:** TODA ideia que vira pauta passa pelo {{APP_PESSOAL}}. Sem rascunho em MD solto.
...
## Status válidos (enum {{APP_PESSOAL}})
...
**Razão:** Quando {{OPERADOR}} passa ideia direta pela Jade, é sempre "ideia fresca que quero gravar logo" — vai pro **topo da fila** automaticamente. Status `pronto_para_gravar` é canônico TANTO em `youtube` (validado via PATCH) QUANTO em `vertical` (default do POST). Mover `ideia_crua` ou `proximos` quando {{OPERADOR}} passou direto = falha de processo.
...
```markdown
Ideia criada no {{APP_PESSOAL}}

- ID: {id}
- URL admin: https://{{app_pessoal}}.{{handle}}.com/{{handle}}/conteudos/{id}
- Formato: {formato}
- Status: {status}
...
- Card visível em `https://{{app_pessoal}}.{{handle}}.com/{{handle}}/conteudos/{id}`
...
6. **admin_url retorna null em youtube/vertical** — montar manualmente: `https://{{app_pessoal}}.{{handle}}.com/{{handle}}/conteudos/{id}`
