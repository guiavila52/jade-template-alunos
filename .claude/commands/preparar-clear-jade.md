...
📎 Versão deste projeto da família `/preparar-clear-{projeto}`. Sister: `/preparar-clear-{{FERRAMENTA_CONTEUDO}}` (outro projeto).
...
Pendências vivem no **ClickUp** list `{{CLICKUP_LIST_ID}}` ("Tasks Jade COO"). Arquivo `workspace/memory/pendencias.md` é ponteiro de 15 linhas — não atualizar manualmente.
...
3. Mudar status: `em progresso` → `aprovação` (espera operador) OU `concluído` (se task auto-fechável)
...
Verifique o que surgiu nesta sessão que vale guardar em `~/.claude/projects/{{PROJECT_PATH_HASH}}/memory/`:

| Tipo | Arquivo | Conteúdo |
|---|---|---|
| Feedback recorrente do operador | `feedback_*.md` | Correções, preferências, aprovações de abordagem |
| Decisão de produto / arquitetura | `project_*.md` | Mudanças no posicionamento, squad, integrações |
| Info nova sobre operador | `user_*.md` | Cargo, contexto pessoal, preferências fixas |
...
Tema novo só com aval explícito do operador.
...
- Reincidência (operador pediu 2+ vezes a mesma correção)
- OU operador pediu explicitamente promoção
...
## Passo 5 — (removido em 2026-06-12, aval operador)

PROGRESS.md foi descontinuado — progresso e pendências vivem SÓ no ClickUp (Regra §1). Não criar, não procurar, não reportar este arquivo.

---

## Passo 6 — Log rotina autônoma (se aplicável)

Se a sessão foi disparada por `/rotina-operador-ausente` E ainda EM CURSO (não declarou Caqui):
...
Antes de mandar a resposta padronizada (Passo 9), executar bateria de verificação real. SÓ se TODOS os checks passarem é que a resposta inclui a frase "tá tudo em paz ✌️". Se algum falhar, NÃO mandar a frase — listar o que ficou pendente e pedir pro operador resolver antes de `/clear`.
...
CLICKUP_OK=$(curl -s -o /dev/null -w "%{http_code}"   -H "Authorization: $CLICKUP_API_TOKEN"   "https://api.clickup.com/api/v2/list/{{CLICKUP_LIST_ID}}/task?archived=false&include_closed=false&page=0")

# 4. Memória persistente acessível (auto-memory MEMORY.md existe)
MEMORY_INDEX="$HOME/.claude/projects/{{PROJECT_PATH_HASH}}/MEMORY.md"
...
RECENT_MEM=$(find "$HOME/.claude/projects/{{PROJECT_PATH_HASH}}/memory" -name "*.md" -mmin -360 2>/dev/null | wc -l | tr -d ' ')
...
Se QUALQUER falhar → listar exatamente qual check falhou + pedir ação ao operador antes de `/clear`. **NÃO escrever a frase de paz com o emoji ✌️ sem rodar a bateria.**
