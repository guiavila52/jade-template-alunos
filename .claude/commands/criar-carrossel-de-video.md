...
description: Atalho ponta-a-ponta. Input = URL YouTube. Encadeia transcrição → estratégia → copy slide-a-slide → lâminas HTML→PNG → triple-check. Output: pasta workspace/output/carrosseis/YYYY-MM-DD-[slug]/ completa. Sem fricção, sem perguntar — {{NOME_OPERADOR_CURTO}} delegou.
...
{{NOME_OPERADOR_CURTO}} manda URL YouTube e quer carrossel pronto SEM ser perguntado nada. Atalho:
...
- `/criar-carrossel-de-video` é específica e NÃO pergunta — {{NOME_OPERADOR_CURTO}} já delegou
...
| **Jade → estrategista** | transcrição completa + ICP do {{NOME_OPERADOR_CURTO}} | briefing-estrategista.md (ângulo + narrativa + payoff + qtd lâminas + estrutura + tom + refs) |
...
- Métricas públicas: nunca expor faturamento (Regra `feedback_metricas_publicas.md`)
...
- Se algum agente reprovar 2x seguidas: pausa e relata ao {{NOME_OPERADOR_CURTO}} (Regra #9 — segundo fracasso muda perspectiva)

## Fluxo

```
[ {{NOME_OPERADOR_CURTO}} dispara /criar-carrossel-de-video <URL> ]
        ↓
[ Jade valida URL ]
   │
...
[ estrategista (squad-gestao) ]
   recebe: transcricao.txt + ICP {{NOME_OPERADOR_CURTO}} + memórias
...
Quando {{NOME_OPERADOR_CURTO}} aprovar/rejeitar:
...
**O que funcionou:** [o que {{NOME_OPERADOR_CURTO}} aprovou]
...
**O que não funcionou:** [o que {{NOME_OPERADOR_CURTO}} rejeitou]
