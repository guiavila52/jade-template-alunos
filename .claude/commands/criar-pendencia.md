...
- {{NOME_OPERADOR}} passa uma demanda nova ("preciso que vc faça X")
...
**Regra §1 (AGENTS.md):** TODA demanda do {{NOME_OPERADOR}} (mesmo "pequena") vira pendência ANTES de executar. Sem exceção.
...
- **NÃO CONFUNDIR com:** `901327190242` (Gimmick backlog — sessão Gimmick paralela, NÃO usar daqui)
...
- `aprovacao-gui` — pendente input/decisão {{NOME_OPERADOR}}
- `skill-faltante` — skill a criar/refinar
- `arquitetura` — mudança estrutural
- `financeiro`, `nf`, `trafego`, `meta-ads`, `gimmick`, `paginas`, `astro`, `migracao`, `seguranca`, `infra`, `dns`
...
- Description tem contexto suficiente pra outro agente executar sem voltar pro {{NOME_OPERADOR}}
...
Se a demanda **veio de um print/screenshot do {{NOME_OPERADOR}}**, anexar via REST:
...
- Se feedback do {{NOME_OPERADOR}} apontou skill faltando: criar pendência cascata (criar/ajustar a skill, com aval {{NOME_OPERADOR}} §13)
