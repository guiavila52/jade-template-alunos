...
- Gui aprovou o output final
- Pendência virou obsoleta (mudança estratégica, deduplicação)

**NÃO usar:** se entrega for parcial (use `/comentar-pendencia`).
**NÃO usar:** se Gui ainda precisa aprovar (use `/comentar-pendencia` notificando "aguarda aprovação Gui").
...
**Validação:**
- {como Gui pode verificar — preview, build, link, etc}
...
- HTTP 401 → token inválido → reportar Gui
...
- Se Gui reabrir um task fechado: criar pendência de retrofit + atualizar aprendizado "fechamento prematuro"
