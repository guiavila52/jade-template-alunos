...
**Método de referência:** Light Copy — narrativa que envolve antes de persuadir. O documento do método está no banco de histórias quando chegar.
...
- {{PLATAFORMA_CURSOS}}: {{METRICA_PLATAFORMA}} / cofundada com {{NOME_COFUNDADOR_PLATAFORMA}}
- {{PRODUTO_PARCERIA}}: {{DESCRICAO_PRODUTO_PARCERIA}}
- Tempo de mercado / origem ({{ORIGEM_OPERADOR}})
...
- "{{METRICA_PLATAFORMA}}+ usuários {{PLATAFORMA_CURSOS}}" — leitor confunde com clientes do {{NOME_OPERADOR_CURTO}} (são alunos dos clientes da {{PLATAFORMA_CURSOS}})
...
- "Avaliação média X.X nos cursos" (se métrica auditável existir — usar só números reais e auditáveis do seu negócio)
- "Cofundador da {{PRODUTO_PARCERIA}} — {{DESCRICAO_PRODUTO_PARCERIA}}"
- Premiações pessoais, certificações, reconhecimentos

**Dúvidas frequentes:**
- "Posso falar dos {{METRICA_PLATAFORMA}} usuários da {{PLATAFORMA_CURSOS}}?" → SIM, mas qualificando ("plataforma com {{METRICA_PLATAFORMA}}+ usuários ativos hospedando cursos") — NÃO como prova social do {{NOME_OPERADOR_CURTO}} em LP da consultoria/mentoria dele
