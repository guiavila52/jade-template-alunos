...
- `{{GTM_CONTAINER_ID}}` count >= 2 (head script + noscript iframe — Regra §5)
...
for u in "${URLS[@]}"; do
  ...
  GTM=$(printf '%s' "$HTML" | grep -o '{{GTM_CONTAINER_ID}}' | wc -l | tr -d ' ')
...
- `MEDIUM | M2 | página /sua-pagina referencia 04-decisoes/decisao-antiga.md mas arquivo não existe — citação quebrada`
