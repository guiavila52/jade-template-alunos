...
- {{NOME_OPERADOR_CURTO}} aprovou/rejeitou parte do output → registrar decisão
- Cross-reference: linkar memória/decisão/aprendizado criado pela pendência
- {{NOME_SUPORTE}} mencionou algo num task — Jade responde via comment

## Inputs

| Campo | Tipo | Obrigatório |
|---|---|---|
| `task_id` | string (ex: `{{CLICKUP_TASK_EXEMPLO}}`) | sim |
| `comment_text` | string (markdown OK) | sim |
| `notify_all` | boolean | opcional (default `false`) |

## IDs canônicos

- List ID `{{CLICKUP_LIST_ID}}` é o esperado. Skill aceita qualquer task_id válido — mas se o task não pertencer a essa lista, a skill **avisa** (proteção contra mexer em listas externas como {{PLATAFORMA_NEWSLETTER}} `{{CLICKUP_LIST_ID_2}}`).
...
- **Decisão {{NOME_OPERADOR_CURTO}}:** `{{NOME_OPERADOR_CURTO}} aprovou {output} em {data}. Próximo: {ação}.`
...
- Se {{NOME_OPERADOR_CURTO}} disser "esse comentário tá confuso/longo demais": registrar em `squads/gestao/aprendizados.md` + ajustar convenção acima
