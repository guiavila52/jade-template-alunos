...
>
> Acionada por `/criar-pagina-nova` (passo 2), por `/jade` (quando há decisão de produto/oferta) ou diretamente pelo {{NOME_OPERADOR_CURTO}}.
...
5. **MEMORY.md index** em `~/.claude/projects/<seu-project-hash>/memory/
  # Para descobrir o project-hash: rode `ls ~/.claude/projects/` e copie a pasta do seu projetoMEMORY.md` + memórias relevantes pra essa estratégia específica:
   ...
   - SE MENCIONAR FUNIL: `project_jornada_cliente.md`
...
7. **`segundo-cerebro/01-identidade/banco-de-historias.md`** (se existir) — histórias canônicas do {{NOME_OPERADOR_CURTO}} pra usar como gancho.
...
- **Objetivo da estratégia** (ex: "página /mentoria nova", "campanha de aquecimento {{PRODUTO_ENTRADA}} de 21/05", "repositioning {{PRODUTO_PRINCIPAL}}")
...
Se a estratégia depende de decisões que SÓ o {{NOME_OPERADOR_CURTO}} pode dar: listar no campo "Decisões pendentes" (seção 11). Bloqueia despacho consequente até resposta.
...
### 6. Após aprovação do {{NOME_OPERADOR_CURTO}} — verificação consequente
...
- `squads/conteudo/aprendizados.md` — se houve correção do {{NOME_OPERADOR_CURTO}} na revisão, registrar aprendizado
...
- [ ] Decisões pendentes estão listadas (não escolheu pelo {{NOME_OPERADOR_CURTO}})
...
- **Não escolher pelo {{NOME_OPERADOR_CURTO}}** decisões estratégicas — listar como pendente.
...
Se input foi transcrição YouTube: estrategista identifica **3 candidatos de ângulo único** (não 1 — {{NOME_OPERADOR_CURTO}} escolhe o melhor) e recomenda 1 com justificativa.
