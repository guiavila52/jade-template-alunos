...
NUNCA `headless: false` nem `devtools: true`. Browser visível no Mac do {{NOME_OPERADOR}} atrapalha trabalho dele.
...
- **design_system:** path do design system aplicado (ex: `workspace/design-systems/{{github_user}}-premium.md`)

**Exemplo:**
```
/revisar-visual-pagina --path=src/pages/mentoria/index.astro --url_local=http://localhost:4321/mentoria --url_prod_atual=https://sites.{{DOMINIO}}/mentoria --briefing=workspace/output/estrategia/2026-05-14-mentoria-redesign-briefing.md --design_system=workspace/design-systems/{{github_user}}-premium.md
```
...
2. **Tipografia**
   ...
   - Números/preços/datas SEMPRE em Inter Tight tabular-nums ou JetBrains Mono (display jamais em números — {{NOME_OPERADOR}} 18/05/2026, memória feedback_fonte_display_jamais_em_numeros)
...
- Falsa aprovação detectada pelo {{NOME_OPERADOR}} → registrar em `squads/dev/agentes/designer-revisor/aprendizados.md` + atualizar critérios
