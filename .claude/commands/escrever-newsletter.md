...
description: Produz e publica a newsletter semanal do {{NOME_OPERADOR}} — compartilhar conhecimento com a base. CTA/pitch é opcional, só adicionado quando {{NOME_OPERADOR_CURTO}} pede.
type: skill
---
...
| **Jade** | Sobe ao {{PLATAFORMA_NEWSLETTER}} via API e valida | — |
...
```
[ {{NOME_OPERADOR_CURTO}} pede newsletter ]
        ↓
...
[ Jade despacha @designer-revisor com path do HTML ]
   ...
   REPROVADO? → corrigir markdown → re-renderizar → revisão de novo
        ↓
[ Jade apresenta HTML ao {{NOME_OPERADOR_CURTO}} — NUNCA markdown ]
        ↓
   {{NOME_OPERADOR_CURTO}} aprova copy? → Jade sobe ao {{PLATAFORMA_NEWSLETTER}} via API → valida GET
        ↓
[ Jade roda publicar-ghl.py → cria rascunho no GHL + envia teste pra {{EMAIL_OPERADOR}} ]
        ↓
[ {{NOME_OPERADOR_CURTO}} recebe email teste, confirma OK no chat ]
        ↓
[ {{NOME_OPERADOR_CURTO}} agenda manualmente no painel GHL ]
   {{NOME_OPERADOR_CURTO}} ajusta copy? → @copywriter corrige → re-renderizar → abrir edit.html → aguardar aprovação
   ⚠️ NÃO rodar publicar-ghl.py durante iteração de copy — só após aprovação final
```

**Regra inviolável (21/05/2026):** {{NOME_OPERADOR_CURTO}} **nunca** valida markdown. Toda aprovação acontece em HTML renderizado.
...
- **Audiência:** `minha-base` ou `{{plataforma_cursos}}` (define variante do PS)
...
2. `segundo-cerebro/01-identidade/tom-de-voz.md` — como o {{NOME_OPERADOR_CURTO}} fala e o que nunca dizer
...
[Pitch/CTA — só se {{NOME_OPERADOR_CURTO}} pediu]
...
### CTA/Pitch (OPCIONAL — só quando {{NOME_OPERADOR_CURTO}} pedir)

Se {{NOME_OPERADOR_CURTO}} pediu pitch, adicionar 1–2 frases antes de "Um abraço,":
...
```
**PS:** Se você quiser [referência ao tema aplicado à {{PLATAFORMA_CURSOS}}], dá uma olhada no que a gente tá construindo lá na [{{PLATAFORMA_CURSOS}}](https://{{DOMINIO}}/{{plataforma_cursos}}).
```
...
- Nunca misturar: newsletter pra {{PLATAFORMA_CURSOS}} nunca menciona o {{PRODUTO_PRINCIPAL}}
...
## Subida ao {{PLATAFORMA_NEWSLETTER}} (após aprovação visual)
...
# POST para https://{{plataforma_newsletter}}.{{DOMINIO}}/api/content/newsletters
# Auth: Authorization: Bearer ${{{PLATAFORMA_NEWSLETTER_API_KEY}}}
```
...
## Publicação no GHL (após {{PLATAFORMA_NEWSLETTER}} aprovado)
...
- {{NOME_OPERADOR_CURTO}} recebe o email de teste, confere no inbox
- {{NOME_OPERADOR_CURTO}} dá **OK no chat** confirmando que o email chegou certo
- {{NOME_OPERADOR_CURTO}} acessa o painel GHL e agenda o disparo (newsletter = toda **terça-feira 09:00 BRT**)

⚠️ **REGRA INVIOLÁVEL:** `publicar-ghl.py` roda UMA única vez — quando {{NOME_OPERADOR_CURTO}} aprova a copy final.
Durante iteração de copy (ajustes, revisões), Jade renderiza o HTML e abre o `-edit.html` para {{NOME_OPERADOR_CURTO}} revisar localmente. NUNCA rodar publicar-ghl.py a cada ajuste de copy.

**Chave Resend:** `RESEND_API_KEY` em `app/.env.local`. Se expirada → {{NOME_OPERADOR_CURTO}} regenera em resend.com/api-keys e atualiza o .env.local.

**Chave GHL:** `GHL_API_KEY` em `app/.env.local` (PIT token). Se retornar 401 → {{NOME_OPERADOR_CURTO}} regenera em GHL → Private Integrations.

---

## Apresentação ao {{NOME_OPERADOR_CURTO}}

- Mostrar **path do preview.html** + abrir no browser (`open`)
- Mostrar **link admin {{PLATAFORMA_NEWSLETTER}}** (após subida)
- **Nunca** mostrar markdown ao {{NOME_OPERADOR_CURTO}}

---

## Aprendizado (após aprovação ou rejeição do {{NOME_OPERADOR_CURTO}})
...
- ❌ Gatilho-resposta: "Responde com X", "Envia Y pra receber Z" — proibido desde {{DATA_EVENTO}}
- ❌ "Conhecimento é commodity" — deprecia o produto do {{NOME_OPERADOR_CURTO}} (usar "informação é commodity")
- ❌ Métricas de receita em R$ no body
- ❌ Pitch sem {{NOME_OPERADOR_CURTO}} ter pedido
- ❌ Markdown entregue ao {{NOME_OPERADOR_CURTO}} sem renderizar