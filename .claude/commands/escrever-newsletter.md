...
- **PROIBIDO gatilho-resposta com palavra-chave.** Nunca incluir "Responde com [PALAVRA]", "Envia [PALAVRA] pra receber", "Comenta [PALAVRA]". Newsletter tem CTA único (vídeo no YouTube ou link único), não duplo. Decisão {{NOME_OPERADOR}} 14/05/2026.
...
⚠️ **segundo-cerebro = só leitura.** Consulte os arquivos de identidade e negócios para contexto, mas nunca edite nenhum arquivo dentro de `segundo-cerebro/`. Edições no cérebro são feitas apenas pelo COO (Jade) com instrução explícita do {{NOME_OPERADOR}}.

---

## Estrutura canônica obrigatória (IMPOSITIVO)

Decisão {{NOME_OPERADOR}} 12/05/2026: TODA newsletter segue O MESMO template. Sem variação.
...
- v1: primeira versão
- v2: ajustes após feedback do {{NOME_OPERADOR}}
...
```
...
- O renderer (`scripts/newsletter/renderizar-html.py` → `renderizar_assinatura()`) monta automaticamente: foto 96x96 circular + 4 linhas canônicas ({{NOME_OPERADOR}} bold / Fundador e CEO da {{EMPRESA_COFUNDADA}} / Autor do Sistema Reverso, Automações PRO e ClickUp 8x / Fundador do {{EMPRESA_NEGOCIO}} · {{DOMINIO}}).
...
### Bullets semânticos
- `<ul><li>` HTML semânticos (Gimmick agora respeita — bugs corrigidos 12/05/2026)
...
3. **Bloco convite** — 1-2 frases convidando pro Sistema Reverso + Mentoria com hyperlinks:
   - Sistema Reverso: https://{{DOMINIO}}/reverso
   - Mentoria: https://{{DOMINIO}}/mentoria
4. **"Um abraço,"** (vírgula, NUNCA exclamação)

**Frase forte:**
- Síntese contundente, "moral da história" do email
- Light Copy, sem floreio, estilo {{NOME_OPERADOR}}
...
- Hyperlinks obrigatórios em AMBOS os produtos (Sistema Reverso + Mentoria)
- 1-2 frases concisas, integradas ao texto (NÃO bullet list)
- Exemplos canônicos:
  - "Quem quer fazer junto comigo: tem o [Sistema Reverso](https://{{DOMINIO}}/reverso) (curso completo) e a [mentoria](https://{{DOMINIO}}/mentoria) (sessões ao vivo)."
  - "Se ressoou e quer aplicar com método: [Sistema Reverso](https://{{DOMINIO}}/reverso) ou [mentoria comigo](https://{{DOMINIO}}/mentoria)."
  - "Se quiser destravar na prática, dá uma olhada no [Sistema Reverso](https://{{DOMINIO}}/reverso) ou entra direto na [mentoria](https://{{DOMINIO}}/mentoria) — a gente acelera."

Decisão {{NOME_OPERADOR}} 14/05/2026. Estrutura imutável. Ver memória `feedback_newsletter_fechamento_canonico_final.md`.
...
[ 2. Perguntar tema/insight da semana ] → @newsletter
        ↓
   ⟶ aguarda input do {{NOME_OPERADOR}}
        ↓
...
### 8 bugs estruturais Gimmick (já corrigidos pela sessão paralela — commits 6ef59c9 + 2f9c581)
...
8. {{NOME_OPERADOR}} valida Gmail → GO
9. Disparo base
...
**Posicionamento (obrigatório):** SEMPRE no fechamento canônico v3, como PRIMEIRO dos 4 elementos do fechamento (vídeo → frase forte → convite Reverso+Mentoria → "Um abraço,"). NUNCA no meio do corpo. Decisão {{NOME_OPERADOR}} 14/05/2026 — fechamento canônico v3 sobrepõe a regra antiga de "após primeiro parágrafo".
...
Newsletter `cadc4df0-21e2-4b0b-84d8-adb517ab1275` (transcrição YouTube) foi criada SEM bloco vídeo. {{NOME_OPERADOR}} apontou imediatamente. Root cause: skill não exigia explicitamente.

**Lição:** Quando input é vídeo (transcrição), embed do vídeo é PARTE DO CONTEÚDO (não opcional). Leitor quer assistir a fonte.

---

## Passo final obrigatório — criar no Gimmick via API REST com HTML PRÉ-RENDERIZADO

**Decisão {{NOME_OPERADOR}} 14/05/2026.** Toda newsletter dessa skill termina criando no Gimmick — mas o que vai no campo `body` da API é o **HTML pré-renderizado** pelo `scripts/newsletter/renderizar-html.py`, NÃO o markdown raw.

### Por quê (não improvisar — bug histórico confirmado 14/05/2026)

- O Gimmick renderiza o conteúdo do campo `body` **como está**. Ele NÃO interpreta o bloco `::video-embed::` (sintaxe nossa) e NÃO adiciona assinatura sozinho.
...
- Conclusão: **sempre rodar o renderer local antes do POST/PATCH no Gimmick**.
...
6. POST/PATCH no Gimmick com `body=<conteúdo do preview.html>` (NÃO o markdown)
7. Validação obrigatória: fazer GET na newsletter recém-criada e confirmar que `newsletter_content.body` começa com `<` e NÃO contém a string literal `::video-embed::` nem `<!-- INTERNO`. Se contiver, falhou — reportar pro {{NOME_OPERADOR}}.

### Endpoint

- **Criar:** `POST https://gimmick.{{DOMINIO}}/api/content/newsletters`
- **Atualizar:** `PATCH https://gimmick.{{DOMINIO}}/api/content/newsletters/{id}`
- **Validar:** `GET https://gimmick.{{DOMINIO}}/api/content/newsletters/{id}` → checar `data.newsletter_content.body`
...
req = urllib.request.Request(
    "https://gimmick.{{DOMINIO}}/api/content/newsletters",
...
with urllib.request.urlopen(req, timeout=30) as r:
    ...
    print(f"Gimmick ID: {nl_id}")
    print(f"Admin URL: {admin_url}")

# VALIDAÇÃO OBRIGATÓRIA — não pular
req2 = urllib.request.Request(
    f"https://gimmick.{{DOMINIO}}/api/content/newsletters/{nl_id}",
...
- **Conhecimento** = produto (sequenciado, filtrado, organizado, contextualizado — o que {{NOME_OPERADOR}} vende)

**Why:** Frase "Conhecimento é commodity" deprecia exatamente o que o {{NOME_OPERADOR}} vende (curso, mentoria, sistema). {{NOME_OPERADOR}} rejeitou em newsletter v2 Mapa Digital 2026 e trocou pra "Informação é commodity. Organização é produto."
...
**Decisão {{NOME_OPERADOR}}:** TODA newsletter sai em 2 versões paralelas com corpo idêntico. SÓ muda o último parágrafo (pitch).

### Matriz canônica

| Versão | Público (tag GHL) | Pitch | Link único |
|---|---|---|---|
| **A — não-aluno** | sem tag Sistema Reverso | Sistema Reverso (curso + comunidade — destacar comunidade) | {{DOMINIO}}/reverso |
| **B — aluno** | com tag Sistema Reverso | Mentoria (encontros semanais ao vivo com {{NOME_OPERADOR}} em grupo) | {{DOMINIO}}/mentoria |

### Estrutura do pitch — não-aluno (Sistema Reverso)
- Hook de transição
...
- Encontros ao vivo semanais com {{NOME_OPERADOR}}, em grupo
- "Não é mais aula gravada — é construir junto"
- NÃO mencionar preço (não é público — formulário primeiro)
- NÃO vender Sistema Reverso (já tem)
- 1 link único {{DOMINIO}}/mentoria (formulário)
...
- ❌ Vender Sistema Reverso pra aluno

Memória: `feedback_newsletter_duas_versoes_aluno_vs_nao_aluno.md`
Pendência canônica: 86ahg5f9e


---

## Posicionamento canônico Gimmick (14/05/2026)

**Atribuição:** Gimmick é criação **pessoal do {{NOME_OPERADOR}}** — sempre referir em 1ª pessoa singular.

- ✅ "Gimmick — ferramenta que **eu construí**"
- ❌ "Gimmick, que **a gente construiu**" / "**construímos**" / "**nossa ferramenta**"

**Função canônica:** ferramenta pra **facilitar produção de conteúdo + gerar coisas com IA sem precisar mergulhar em automação** (n8n, Make, Zapier — "a forma antiga"; Gimmick substitui).

**Sistema Reverso × Gimmick:** quando aparece em pitch do curso/mentoria — Gimmick **incluído** + **template multi-agentes prontinho** (não 1 agente solto, time de agentes operando junto).
