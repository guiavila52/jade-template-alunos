...
### 2. Cor e contraste
- Brand consistency: cores da marca usadas (preto/dourado em LP do {{NOME_OPERADOR_CURTO}}, paleta Light Copy nos carrosséis)
...
- `feedback_metricas_publicas.md` — sem expor faturamento
...
Newsletters HTML (fragments gerados via `/renderizar-newsletter-html`) DEVEM passar por validação visual Playwright ANTES de PATCH no {{PLATAFORMA_EMAIL}}.
...
python3 scripts/review/render-and-screenshot.py \
  ...
  --slug {id-newsletter} \
  --{{plataforma_newsletter}}-url https://{{plataforma_newsletter}}.{{DOMINIO}}/{{HANDLE_OPERADOR}}/conteudos/{uuid}
```
...
- [ ] Assinatura canônica do operador presente e correta (ver `segundo-cerebro/01-identidade/` para o formato exato da assinatura)
...
## {{PLATAFORMA_EMAIL}} URL
https://{{plataforma_newsletter}}.{{DOMINIO}}/{{HANDLE_OPERADOR}}/conteudos/{uuid}
...
Hook #38 (`check-revisao-visual-antes-publicar.sh`) bloqueia PATCH body {{PLATAFORMA_EMAIL}} se não encontrar `REVISAO-APROVADO-{data recente}-{slug}.md` na pasta `workspace/output/screenshots-revisao/`.

Bypass legítimo: `export JADE_CONTEXT=desenvolvendo-revisor` (apenas durante criação/teste do próprio script de revisão).

---

## 🆕 PROTOCOLO 2 PASSADAS (2026-05-17 — Task {{CLICKUP_TASK_EXEMPLO}})

### Por que existe
Histórico: validação técnica isolada deixou passar página com 5 bugs visuais óbvios ({{NOME_OPERADOR_CURTO}} pegou em 3 segundos). Falta gate estético ANTES do técnico.

### Passada 1 — ESTÉTICA HUMANA (gate principal)

**SEMPRE rodar primeiro. Se reprovar, NEM RODA passada 2.**

Critérios subjetivos mas críticos (olhar screenshot inteiro como {{NOME_OPERADOR_CURTO}} olharia):
...
**Bug grave reincidente:** {{NOME_OPERADOR_CURTO}} clicou em card do organograma e modal abriu no CANTO SUPERIOR ESQUERDO, fora de centro. Revisor anterior tinha APROVADO porque só validou screenshot estático.

**Cita {{NOME_OPERADOR_CURTO}} literal (frustração no limite):**
...
## 🔴 17/05 NOITE — Calibração "EMOCIONA?" não bate com {{NOME_OPERADOR_CURTO}} (falha grave)

Revisor aprovou hero com Fraunces opticals 144 + drop-cap "S" gradient como "EMOCIONA Linear-tier". {{NOME_OPERADOR_CURTO}} (avaliação humana real) disse "não gostei dessa fonte e desse S desse jeito".

**Aprendizado:** "emociona" subjetivo varia muito entre revisor agent e {{NOME_OPERADOR_CURTO}} real. Critério precisa ser MAIS CONSERVADOR:
...
- Tom {{NOME_OPERADOR_CURTO}} = "leve, funcional, clean, moderno" — NÃO editorial exuberante
