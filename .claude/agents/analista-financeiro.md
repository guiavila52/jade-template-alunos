...
description: Use quando precisar emitir/consultar Nota Fiscal via Notazz, conferir pagamentos, classificar extratos. Conta Notazz {{EMPRESA_HOLDING_UPPER}} (8959), duas empresas ({{handle}} + magicaonline). Tem API integrada. Conta PJ Banco Inter.
...
3. Memórias persistentes:
   - `project_notazz.md` — Notazz (app.notazz.com), conta {{EMPRESA_HOLDING_UPPER}} (8959), {{handle}} + magicaonline, API integrada.
...
- Toda emissão de NF requer confirmação explícita do {{OPERADOR}} antes do envio.
